...
import { getVocabularyLevelItems } from "../../api/vocabularyRoutes";
import type {
  VocabularyItem,
  VocabularyItemState,
...
function groupKey(state: VocabularyItemState): string {
  return `vocRoutes.levelStates.${state}`;
}

interface VocabularyLevelPanelProps {
  userId: string | null;
  level: string;
 ...
 */
export function VocabularyLevelPanel({
  userId,
  level,
  disabled,
  refreshNonce,
  ...
  onReviewLearned,
  onDemonstrate,
}: VocabularyLevelPanelProps) {
  const { t } = useI18n();
  const [data, setData] = useState<VocabularyLevelItems | null>(null);
  const [open, setOpen] = useState<VocabularyItemState>("failed");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const items = await getVocabularyLevelItems(uid, level);
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          <Badge variant="outline" className="gap-1">
            {data.total} {t("vocRoutes.levelItemsChecks")}
          </Badge>
          {shortBank && (
            <Badge variant="outline" className="border-warning/40 text-warning">
              {t("vocRoutes.shortBank")}
            </Badge>
          )}
          <span className="text-muted-foreground">
            {t("vocRoutes.levelItemsSummary")
      ...
      {shortBank && (
        <div className="rounded-lg border border-warning/30 bg-warning/5 px-3 py-2.5 text-xs">
          <p className="font-medium text-warning">
            {t("vocRoutes.shortBankTitle").replace("{level}", level)}
          </p>
          <p className="mt-1 leading-relaxed text-muted-foreground">
            {t("vocRoutes.shortBankNote").replace("{total}", String(data.total))}
          </p>
        </div>
      )}
      ...
      {showGate && (
        <div className="rounded-lg border border-warning/30 bg-warning/5 px-3 py-2.5 text-xs">
          <p className="font-medium text-warning">
            {t("vocRoutes.routeGateIntro").replace("{level}", level)}
          </p>
          <p className="mt-1 leading-relaxed text-muted-foreground">
            {t("vocRoutes.routeGateLine")
              ...
              >
                {group.items.length === 0 ? (
                  <li className="px-2 py-1 text-xs text-muted-foreground">
                    {t("vocRoutes.levelItemsEmpty")}
                  </li>
                ) : (
                  group.items.map((item) => (
                    <VocabularyItemRow key={item.check_id} item={item} />
                  ))
                )}
              </ul>
            )}
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-border pt-3">
        ...
        >
          {t("vocRoutes.repeatFailed").replace("{count}", String(data.failed))}
        </Button>
        <Button
          type="button"
          variant="outline"
        ...
        >
          {t("vocRoutes.reviewLearned").replace(
            "{count}",
        ...
        >
          {data.completed
            ? t("vocRoutes.reviewLevel").replace("{level}", level)
            : t("vocRoutes.practiceLevel").replace("{level}", level)}
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-primary/25 bg-primary/5 px-3 py-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs font-semibold text-foreground">
            {t("vocRoutes.demonstrateTitle").replace("{level}", level)}
          </p>
...
function VocabularyItemRow({ item }: { item: VocabularyItem }) {
  const { t } = useI18n();
  return (
    <li className="flex items-start justify-between gap-3 rounded-md px-2 py-1.5 hover:bg-accent/60">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 flex flex-wrap gap-x-2 text-[10px] uppercase tracking-wide text-muted-foreground">
          ...
          {item.attempts > 0 && (
            <span>
              {t("vocRoutes.levelItemsAttempts")}: {item.attempts}
            </span>
          )}
        </p>
      </div>
