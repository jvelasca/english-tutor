...
import { getConversationLevelItems } from "../../api/conversationRoutes";
import type {
  ConversationItem,
...
const GROUPS: ConversationItemState[] = ["failed", "mastered", "unseen"];

function groupKey(state: ConversationItemState): string {
  return `convRoutes.levelStates.${state}`;
}

interface ConversationLevelPanelProps {
  userId: string | null;
  level: string;
  /** Estado pedagógico de la ruta (functional ≠ demonstrated). */
  routeState?: ConversationRouteState;
  ...
  /** Repasar lo aprendido: rotación solo por los diálogos ya dominados. */
...
/**
 * Historial por diálogo de un nivel (control del alumno). Al desplegar el donut
 * de un nivel muestra sus mini-diálogos agrupados en fallidos / dominados / sin
 * ver, y ofrece repetir los fallidos hasta dominarlos, repasar lo aprendido
 * (solo dominados) o practicar/repasar el nivel completo. La ruta es práctica:
 * demostrar el nivel exige el Speaking Assessment (evidencia formal oral),
 * nunca esta ruta.
 */
export function ConversationLevelPanel({
  userId,
  level,
  routeState,
  assessedLevel,
  ...
  onReviewLearned,
  onDemonstrate,
}: ConversationLevelPanelProps) {
  const { t } = useI18n();
  const [data, setData] = useState<ConversationLevelItems | null>(null);
  const [open, setOpen] = useState<ConversationItemState>("failed");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const items = await getConversationLevelItems(uid, level);
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-1.5 text-xs">
          <Badge variant="outline" className="gap-1">
            {data.total} {t("convRoutes.levelItemsDialogues")}
          </Badge>
          <span className="text-muted-foreground">
            {t("convRoutes.levelItemsSummary")
        ...
        {data.completed ? (
          <Badge className="gap-1">{t("convRoutes.completedShort")}</Badge>
        ) : (
          pendingCert && (
            <Badge variant="outline" className="border-warning/40 text-warning">
              {t("convRoutes.routePendingCert")}
            </Badge>
          )
        )}
      </div>
      ...
      {data.completed && routeState === "functional" && !demonstrated && (
        <div className="rounded-lg border border-warning/30 bg-warning/5 px-3 py-2.5 text-xs">
          <p className="font-medium text-warning">
            {t("convRoutes.demoNotYet").replace("{level}", level)}
          </p>
          <p className="mt-1 leading-relaxed text-muted-foreground">
            {t("convRoutes.demoRequires").replace("{level}", level)}
          </p>
        </div>
      )}
      {demonstrated && (
        <div className="rounded-lg border border-success/30 bg-success/5 px-3 py-2.5 text-xs">
          <p className="font-medium text-success">
            {t("convRoutes.demoTitle").replace("{level}", level)}
          </p>
          <p className="mt-1 leading-relaxed text-muted-foreground">
            {t("convRoutes.demoMet")}
          </p>
        </div>
      )}
      ...
      {showGate && (
        <div className="rounded-lg border border-warning/30 bg-warning/5 px-3 py-2.5 text-xs">
          <p className="font-medium text-warning">
            {t("convRoutes.routeGateIntro").replace("{level}", level)}
          </p>
          <p className="mt-1 leading-relaxed text-muted-foreground">
            {t("convRoutes.routeGateLine")
      ...
      <div className="flex flex-wrap items-center gap-2 border-t border-border pt-3">
        ...
        >
          {t("convRoutes.repeatFailed").replace("{count}", String(data.failed))}
        </Button>
        <Button
          type="button"
          variant="outline"
        ...
        >
          {t("convRoutes.reviewLearned").replace(
            "{count}",
        ...
        >
          {data.completed
            ? t("convRoutes.reviewLevel").replace("{level}", level)
            : t("convRoutes.practiceLevel").replace("{level}", level)}
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-2 rounded-lg border border-primary/25 bg-primary/5 px-3 py-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="text-xs font-semibold text-foreground">
            {demonstrated
              ? t("convRoutes.demoTitle").replace("{level}", level)
              : t("convRoutes.demonstrateTitle").replace("{level}", level)}
          </p>
          {assessedLevel && (
            <Badge variant="outline" className="gap-1">
        ...
        <InfoDisclosure
          id={`conv-demonstrate-note-${level}`}
          label={t("common.moreInfo")}
        >
          <p>{t("convRoutes.demonstrateNote").replace("{level}", level)}</p>
        </InfoDisclosure>
        <div>
          <Button
            type="button"
    ...
    <li className="flex items-start justify-between gap-3 rounded-md px-2 py-1.5 hover:bg-accent/60">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 flex flex-wrap gap-x-2 text-[10px] uppercase tracking-wide text-muted-foreground">
          ...
          {item.attempts > 0 && (
            <span>
              {t("convRoutes.levelItemsAttempts")}: {item.attempts}
            </span>
          )}
        </p>
      </div>
