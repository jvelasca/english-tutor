...
def _empty_learner_level_state() -> dict:
    """Estado de nivel neutro (sin caché): fachada del drill (V3.56).

    La ÚNICA implementación de la lectura del estado vive en
    `domain.learner_state`, compartida por la cola de repaso y el drill para que
    no puedan divergir (premisa 10). Nunca lanza.
    """
    return learner_state_domain.empty_learner_level_state()


async def _learner_level_state(user_id: str) -> dict:
    """Estado de nivel del alumno desde la caché del Student Model (V3.52 → V3.56).

    Fachada estable del drill: delega en
    `domain.learner_state.learner_level_state`, la única implementación de la
    lectura O(1) que V3.56 comparte con la cola de repaso. Nunca lanza.
    """
    return await learner_state_domain.learner_level_state(user_id)
...
def _skill_floor(learner_state: dict, skill: object) -> tuple[str, str]:
    """Suelo (nivel, fuente) de la modalidad que la tarea mide (V3.54 → V3.56, pura).

    Fachada del drill: delega en `services.student_state.skill_floor`, la única
    implementación de la política (mapa por skill con caída al suelo global),
    compartida con el planner del léxico (V3.56). Nunca lanza.
    """
    return student_state.skill_floor(learner_state, skill)
