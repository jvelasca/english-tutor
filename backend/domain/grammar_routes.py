"""Dominio de las rutas de grammar (V3.12).

Orquesta las APIs de práctica MC determinista por nivel CEFR basada en los checks
de grammar del currículo oficial (`objectives[].checks` con skill "grammar"):
siguiente pregunta por bucket (all/failed/mastered), estado del nivel e intentos
(un POST /attempt con la opción elegida se puntúa al instante: acierto ⇒
superado). El motor puro compartido con Vocabulary vive en `services.quiz_routes`
(parametrizado por skill); aquí solo se ata a grammar y a su tabla de intentos.
...
# El motor compartido lanza los ValueErrors con el prefijo de vocabulary (la
# skill que lo estrenó). Grammar los remapea a su propio namespace para que los
# mensajes/handlers de error del router sean coherentes con la skill.
_NO_FAILED = "vocabulary.no_failed"
...
async def next_question(
    ...
    try:
        ...
        if str(exc) == _NO_FAILED:
            raise ValueError("grammar.no_failed") from None
        raise
...
async def get_stats(user_id: str) -> dict:
    """Progreso del mapa de rutas de grammar (niveles + puertas honestas)."""
    attempts_rows = await run_in_threadpool(grammar_repo.list_attempts, user_id)
