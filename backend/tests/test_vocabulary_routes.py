"""Tests de las rutas de vocabulary (V3.11).

Cubre el motor compartido quiz (pool desde checks del currículo, estados por
check, puerta de ruta honesta con checkpoint adaptado a bancos cortos, modos
all/failed/mastered), el repositorio, el dominio determinista y los endpoints.
"""
import asyncio
...
from domain import vocabulary_routes as vocab_domain
from domain.vocabulary_routes import SKILL, is_valid_level
...
def _curriculum_check_ids(level: str) -> set[str]:
    """Ids de los checks de vocabulary del currículo oficial para un nivel."""
    ids: set[str] = set()
...
def test_pool_matches_curriculum_checks():
    for level in engine.LEVEL_ORDER:
        ...
        assert pool, f"nivel {level} sin checks de vocabulary"
...
def test_route_competence_never_demonstrated(monkeypatch, tmp_path):
    ...
    a1 = engine.checks_for_level(SKILL, "A1")
    for c in a1:
        vocab_repo.record_attempt(uid, c["check_id"], "A1", 100.0, True, c["topic"])
    rows = vocab_repo.list_attempts(uid)
...
def test_record_attempt_and_passed_ids(monkeypatch, tmp_path):
    ...
    assert vocab_repo.record_attempt(uid, c["check_id"], "A1", 100.0, True)
    assert vocab_repo.record_attempt(uid, c["check_id"], "A1", 0.0, False)
    assert vocab_repo.passed_check_ids(uid) == {c["check_id"]}
    rows = vocab_repo.list_attempts(uid)
    assert len(rows) == 2
...
def test_submit_attempt_correct_and_wrong(monkeypatch, tmp_path):
    ...
    c = engine.checks_for_level(SKILL, "A1")[0]
    ok = asyncio.run(
        vocab_domain.submit_attempt(uid, c["check_id"], c["correct_index"])
    )
    assert ok is not None
    assert ok["passed"] is True
    assert ok["score"] == 100.0
    wrong = asyncio.run(
        vocab_domain.submit_attempt(
...
def test_submit_attempt_bad_option_raises(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError) as err:
        asyncio.run(vocab_domain.submit_attempt(uid, c["check_id"], 999))
    assert str(err.value) == "vocabulary.bad_option"


def test_submit_attempt_unknown_check_returns_none(monkeypatch, tmp_path):
    uid = _setup(monkeypatch, tmp_path)
    assert asyncio.run(vocab_domain.submit_attempt(uid, "no-existe", 0)) is None
...
def test_question_endpoint_never_leaks_correct_index(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        r = client.get(
            "/api/vocabulary/routes/question", params={"user_id": uid}
        )
        ...
        failed = client.get(
            "/api/vocabulary/routes/question",
...
def test_stats_endpoint_empty(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        r = client.get(
            "/api/vocabulary/routes/stats", params={"user_id": uid}
        )
        assert r.status_code == 200
        body = r.json()
...
def test_attempt_endpoint_scores_and_reveals_answer(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert vocab_repo.list_attempts(uid)  # el intento se persistió
...
def test_attempt_endpoint_bad_option_is_400(monkeypatch, tmp_path):
    ...
    with TestClient(app) as client:
        ...
        assert r.json()["detail"] == "vocabulary.bad_option"
