"""Endpoints de las rutas de vocabulary (V3.11).

Práctica MC determinista por nivel CEFR sobre los checks de vocabulary del
...
from domain import vocabulary_routes as vocabulary_routes_service
from domain.vocabulary_routes import is_valid_level
from schemas.vocabulary_routes import (
    VocabularyAttemptRequest,
    VocabularyAttemptResponse,
    VocabularyLevelItemsOut,
    VocabularyQuestion,
    VocabularyStats,
)
...
@router.get("/api/vocabulary/routes/question", response_model=VocabularyQuestion)
async def question(
    level: str | None = None,
    mode: str = "all",
    ...
    try:
        return await vocabulary_routes_service.next_question(
            user["id"], level=level, mode=mode
        )
    except ValueError as exc:
        if str(exc) == "vocabulary.no_failed":
            raise HTTPException(
                status_code=404, detail="vocabulary.no_failed"
            ) from None
        raise


@router.get(
    "/api/vocabulary/routes/items", response_model=VocabularyLevelItemsOut
)
async def level_items(
    ...
    if not is_valid_level(level):
        raise HTTPException(status_code=400, detail=f"Nivel no válido: {level}")
    return await vocabulary_routes_service.items_for_level_out(user["id"], level)


@router.get("/api/vocabulary/routes/stats", response_model=VocabularyStats)
async def stats(user: dict = Depends(current_user)) -> dict:
    return await vocabulary_routes_service.get_stats(user["id"])


@router.post(
    "/api/vocabulary/routes/attempt",
    response_model=VocabularyAttemptResponse,
)
async def attempt(
    body: VocabularyAttemptRequest,
    user: dict = Depends(current_user),
) -> dict:
    ...
    """
    try:
        result = await vocabulary_routes_service.submit_attempt(
            user["id"], body.check_id, body.selected_index
        )
    except ValueError as exc:
        if str(exc) == "vocabulary.bad_option":
            raise HTTPException(
                status_code=400,
                detail="vocabulary.bad_option",
            ) from None
        raise
    if result is None:
    ...
    await learning_service.record_event(
        user["id"],
        "exercise",
        f"vocabulary:{body.check_id}:{'ok' if result['passed'] else 'ko'}",
    )
    return result
