"""Repositorio de las rutas de vocabulary (SQLite).

Almacena los intentos MC deterministas contra checks del currículo
(`vocabulary_route_attempts`). El estado por check se deriva en vivo desde esa
...
def record_attempt(
    ...
    """Persiste un intento MC de vocabulary para un usuario existente."""
    if get_user(user_id) is None:
        return False
    with closing(_conn()) as conn, conn:
        conn.execute(
            "INSERT INTO vocabulary_route_attempts "
...
def list_attempts(user_id: str) -> list[dict]:
    ...
    with closing(_conn()) as conn:
        rows = conn.execute(
            ...
            "FROM vocabulary_route_attempts "
...
def passed_check_ids(user_id: str) -> set[str]:
    ...
    with closing(_conn()) as conn:
        rows = conn.execute(
            "SELECT DISTINCT check_id FROM vocabulary_route_attempts "
