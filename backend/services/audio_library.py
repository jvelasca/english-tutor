...
- Un **manifest versionado** (`backend/audio_library/manifest.json`) que declara,
  ...
  (hablante, acento, nº de hablantes, ruido, duración, transcripción).
...
class AudioLibraryEntry(BaseModel):
    ...
    `file` es el nombre del WAV dentro de `AUDIO_LIBRARY_DIR` (sin subcarpetas; la
    resolución valida que no escape del directorio). El resto son metadatos
    auditivos reales que respaldan la dificultad declarada del ítem que la
    referencia.
    """
...
def resolve_file(entry: AudioLibraryEntry, base: Path | None = None) -> Path | None:
    ...
    Rechaza rutas que salgan de `base` (p. ej. `../`) para que un manifest malicioso
    o mal formado no pueda leer archivos arbitrarios del sistema.
    """
...
def library_summary(manifest: AudioLibraryManifest | None = None) -> list[dict]:
    """Metadatos del manifest (sin audio) para exponer qué grabaciones existen."""
...
def validate_manifest(manifest: AudioLibraryManifest | None = None) -> list[str]:
    ...
    `file` únicos y no vacíos, y que cada `file` resuelve dentro de la biblioteca.
    """
    m = manifest or load_manifest()
    errors: list[str] = []
