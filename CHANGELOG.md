...
- **Tests y verificación.** Backend pytest **2099 passed** (+15): nuevo `test_context_skill_v350.py` (paridad del vocabulario, skills declaradas/normalizadas, cobertura de cada skill, robustez de `context_skills`, congelación de los originales, no regresión sin `level`/`skill`, preferencia de modalidad con degradación, banda de dificultad en B1 y C2, determinismo, contrato HTTP y paridad GET↔POST con evidencia real por modalidad); se ajusta `test_transfer_cefr_v347.py` (la semántica «con C2 la elección es la de sin nivel» queda superada por el difficulty matching). Frontend vitest **75 ficheros/641 tests** (sin cambios), `ruff`/`tsc` limpios, `npm run build`, `check_beta_v3.py`, `content_validation.py` y `check_release_consistency` **3.50.0** exit 0. **CI 6/6 en verde** (run [34594042697](https://github.com/jvelasca/english-tutor/actions/runs/34594042697) sobre `1c8d6e0`): Release consistency, Backend (ruff + pytest), Frontend (tsc + vitest + build), Playwright E2E (visual), Beta V3.0 gate y Content validation.
