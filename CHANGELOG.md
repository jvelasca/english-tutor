...
- **Tests y verificación.** Backend pytest **2164 passed** en local (+6: `is_test`/filtro por defecto, `DELETE` acotado a test, borrado de filas dependientes y cobertura de `fit`/selector), `ruff` limpio. Frontend vitest **76 ficheros/651 tests** (+1 fichero/+11: `abLoop.test.ts` nuevo y `play` idempotente/rebobinado en `audioController.test.ts`), `tsc --noEmit` y `npm run build` en verde. Lanzador 76 tests. `check_release_consistency` **3.52.1**, `check_beta_v3.py`, `content_validation.py` y `check_i18n_coverage` exit 0.
