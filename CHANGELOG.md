...
**E2E + Adaptive Engine Validation. Release de VALIDACIÓN, no de capacidad: SIN migración, SIN bump de `GENERATOR_VERSION`, SIN tocar el banco, SIN capacidad pedagógica nueva, SIN tocar el argmax del Planner y SIN cambios en producción (el diff de código de producto es CERO).** Demuestra **experimentalmente, por HTTP**, que la cadena `Evidence → Student State → Decision Projection → Task selection → Decision → Serving → Attempt → Outcome → Evidence` funciona **como una sola pieza**, con la batería **E01–E19** (más la cobertura del endpoint huérfano `GET /api/learning/decisions`), y fija el contrato del cliente con una spec de navegador de red mockeada. Es la release que convierte «creemos que el motor adaptativo funciona» en «**está demostrado por escenarios controlados**» antes de entrar en cualquier componente probabilístico (calibración, ELG real).

Versión de app `3.68.0 → 3.69.0`. **Solo tests** (`backend/tests/test_adaptive_e2e_v369.py` nuevo, `frontend/tests/visual/drillProvenance.spec.ts` nuevo) + documentación. **Ninguna línea de código de producto cambia.**
