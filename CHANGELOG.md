...
- **Tests y verificación.** Frontend vitest **75 ficheros/639 tests** (+3 ficheros/+16: `InfoDisclosure`, `selectedRoute` y `useSelectedRoute`) y `microFlow` actualizado al arranque en `while1`. `check_i18n_coverage`: 0 claves indefinidas, 0 duplicadas. `tsc --noEmit`, `npm run build` y `check_release_consistency` **3.48.1** en verde.
