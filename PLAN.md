...
- ✅ **V3.76.0 — El PIN opcional por perfil (Fase 3 del P0 de identidad) + los 9 dossiers de G7 regenerados y reproducibles (2026-09-21)** (**Versión estable `3.76.0`**, app `3.75.8 → 3.76.0`). **Release de PRODUCTO (minor) con la ÚNICA migración de BD del ciclo pre-V4.0**: `users` gana `pin_hash TEXT NOT NULL DEFAULT ''` con el idioma idempotente que ya existía (`PRAGMA table_info` + `ALTER TABLE`), aditiva y con defecto vacío, así que **una BD de V3.75.8 se actualiza sin que nadie pierda el acceso**. **SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el currículum (`CURRICULUM_VERSION` sigue `1.3.1`), SIN tocar las evaluaciones y SIN tocar `LISTENING_BANK_VERSION`.** No añade ni retira un gate: el árbol se congeló en `v3.75.8` y **G1–G7 siguen `pending`**, y el PIN entra **después** de congelar precisamente para no invalidar la validación en curso. **(A) Qué es el PIN y qué no es.** `POST /api/session` acepta un `pin` opcional y responde con **tres desenlaces distinguibles** —`401 PIN_REQUIRED`, `401 PIN_INVALID`, `429 PIN_THROTTLED` con `Retry-After`—, ninguno de los cuales revela si el PIN estaba «cerca». Un perfil sin PIN (`pin_hash = ''`) abre con el cuerpo de siempre: **esto no es autenticación de persona y no cierra el P0 por defecto**, es una mitigación que el dueño **activa**. Así se declara en `PARKED.md` y en las notas: el P0 queda cerrado **para los perfiles que la usan**, no para el producto. **(B) El hash, sin dependencias nuevas.** `backend/services/pins.py` implementa **PBKDF2-HMAC-SHA256** con **200 000 iteraciones**, **sal aleatoria por perfil** e iteraciones declaradas dentro del valor (`pbkdf2-sha256$<iter>$<sal>$<hash>`), de modo que subirlas más adelante no invalida los PIN ya guardados; la comparación es `hmac.compare_digest`. Misma doctrina que `services/sessions.py`: stdlib puro. `User.has_pin` (booleano) es **lo único** que viaja en el perfil; el repositorio lee `pin_hash` para derivarlo y **lo saca del diccionario** (`_row_to_user`) para que no pueda serializarse por descuido, y `get_pin_hash`/`set_pin_hash` son operaciones aparte, porque «quién es este perfil» y «con qué secreto entra» no deben compartir puerta. **(C) El freno de intentos es la pieza que sostiene todo.** Un PIN de 4-6 dígitos es fuerza bruta trivial **sin** freno, así que el freno vive en el mismo módulo que el hash y no como adorno: **5 fallos no frenan** (dedos torpes no son un ataque) y a partir de ahí el retardo **dobla** (1s, 2s, 4s…) con techo de **300 s**, **por perfil** —un perfil bajo ataque no deja fuera a los demás del mismo equipo— y se **limpia al acertar**, que es la mitad que evita convertir el PIN en una condena a la primera equivocación. Mientras el freno está activo, **el PIN correcto tampoco pasa**: si no, el atacante tendría una ventana gratis justo cuando más ha insistido. `/api/session` entra además en `_PATH_LIMITS` (**120/min**) como **primera valla, no como defensa**, declarado así en el código: el cupo por IP es holgado a propósito y una familia tras un NAT comparte IP. **(D) Poner, cambiar y retirar.** `PUT /api/session/pin` (ruta nueva) opera **bajo la sesión**, sin `{id}` en la ruta, así que no existe la forma de tocar el PIN de otro perfil ni por descuido. Cambiar o **retirar** un PIN existente **exige el anterior**, con la misma puerta y el mismo freno: sin eso, quien se sienta ante un equipo con sesión abierta podría poner su propio PIN y quedarse el perfil. `new_pin` vacío lo retira (el perfil vuelve a entrar sin credencial) y `new_pin` con mala forma devuelve **400 `PIN_FORMAT`** sin tocar la BD. Ajustes gana la pestaña **«PIN de este perfil»**, que declara en el propio texto qué protege y qué no. **(E) El arranque deja de fallar en silencio.** Hasta aquí, un `POST /api/session` que no fuera 200 moría en un `catch` vacío; con un perfil con PIN eso habría pasado **siempre** y el alumno habría visto una puerta que no responde, sin un solo mensaje. `planSession` (`utils/session.ts`) gana el desenlace **`pin`** —sigue siendo función pura, así que la matriz entera se prueba sin navegador— y `useChat` centraliza la apertura en **un único camino** (`openProfile`) que usan los **tres** puntos que abren sesión: arranque automático, selector de perfil y alta. Como el cambio de perfil **dentro** de la app también puede pedir PIN, `App` muestra la puerta cuando hay un perfil esperando su PIN **aunque ya hubiera uno activo**: si no, el paso quedaría invisible detrás de la app y el selector parecería roto. **(F) La UI no adivina, y se le puede probar.** `ApiError` (`api/client.ts`) pasa a conservar el **`detail`** y el **`Retry-After`**, y `SessionPinError` los traduce a un desenlace tipado, de modo que la puerta distingue «falta el PIN» —que es el paso siguiente, no una avería— de «PIN incorrecto» y de «freno activo»; antes todos los 401 se convertían en un `Error` de texto y la UI solo podía decir «algo falló». `utils/pin.ts` es **espejo deliberado** de la validación del backend (4-6 dígitos) para no ofrecer un botón que el servidor va a rechazar, y `ProfileGate` **filtra el tecleo a dígitos** en vez de validar «12ab» a posteriori. Quién decide que hace falta PIN es el **servidor** (`has_pin` solo evita el `POST` condenado del arranque): la lista de perfiles puede estar desactualizada, así que la puerta nunca da por hecho lo que no le han confirmado. **(G) G7 preparado, y reproducible.** Los **9 dossiers** de `scripts.audit_dossier` (`corpus-stats`, `curriculum-stats`, `speaking-stats`, `mc-bias`, `cefr-adequacy`, `skill-coverage`, `feedback-coverage`, `mastery-claims`, `assessment-instruments`) se regeneraron sobre el árbol congelado (`v3.75.8` · `7134e538`) y `docs/audit/generated/` quedó **sin un solo byte de diferencia**: los instrumentos versionados describen el árbol que se va a certificar. Es el mejor insumo posible para el veredicto humano de G7 y un hallazgo a registrar si algún día dejara de cumplirse. `record pedagogia` **no** se ha ejecutado: **G7 no se cierra con una regeneración, se cierra con la revisión**. **(H) Verificación.** `tsc --noEmit` limpio, `vitest run` **850/850** (98 ficheros), `npm run build` correcto (el bundle publica `3.76.0`), i18n `--strict` **1533 claves / 0 huérfanas**, contraste `--strict` **480 pares + 6 guardas / 0 bloqueantes**, `ruff` limpio, `pytest` **2942/2942**, `check_release_consistency` OK en los **6 orígenes** (`3.76.0`) y `validation_gate.py auto` **10/10**. Tests nuevos: `backend/tests/test_pin.py` (**31 casos**), `api/session.test.ts` (los cuatro desenlaces del PIN + `setSessionPin`), la matriz de `planSession` con el desenlace `pin`, `ProfileGate.test.tsx`, `utils/pin.test.ts` y `hooks/useChat.test.tsx` (arranque, selector, freno y cancelación). **Honestidad.** (i) **Esto no cierra el P0 por defecto:** un perfil sin PIN sigue entrando sin credencial; es una mitigación que el dueño activa, y el P0 queda cerrado **para los perfiles que la usan**, no para el producto. (ii) **La pieza que carga el peso es el freno, no la longitud:** 4-6 dígitos son fuerza bruta trivial sin él. (iii) **La cookie de un año significa un tecleo por navegador:** protege ante *otro equipo sin la cookie*, no ante quien se sienta delante del tuyo. (iv) **No es autenticación de persona:** no hay recuperación ni identidad; quien olvide el PIN no puede demostrar que es él. (v) **`GET /api/users` sigue enumerando nombres** —ahora con `has_pin`—: consecuencia declarada del producto sin cuentas. (vi) **El hash viaja en el backup** (es estado del perfil, dentro de la BD) mientras `session.secret` sigue sin viajar: decisión declarada, no accidente. (vii) **La evidencia documental del 401 cambia de código:** `test_public_surface.py` gana la ruta nueva en un test propio y no en el diccionario `CON_SESION`, porque esa lista se recorre con `GET` y esta ruta responde **405**, no 401: meterlas juntas habría debilitado el candado del otro lado sin avisar. (viii) **Los 7 gates siguen en `pending`** y la identidad sellada en el kit sigue siendo la del **árbol congelado** (`v3.75.8` · `7134e538`, con el tag `v3.75.8` en `c858e88`), que es la que hay que validar. Ver `release-notes-v3.76.0.md`.

- ✅ **V3.75.8 — El diccionario de consulta con buscador grande y un color por sentido + la versión de compilación en la Ayuda (2026-09-20)** (**Versión estable `3.75.8`**, app `3.75.7 → 3.75.8`). **Release de PRODUCTO (patch): SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el currículum (`CURRICULUM_VERSION` sigue `1.3.1`), SIN tocar las evaluaciones, SIN tocar `LISTENING_BANK_VERSION` y SIN endpoints nuevos** — el diff de producto es **frontend** (`styles/legacy.css`, `utils/dictionaryDirection.ts`, `utils/buildInfo.ts`, `features/vocabulary/*`, `features/help/HelpScreen.tsx`, `utils/i18n.ts`, `scripts/contrast_audit.mjs`). No añade ni retira un gate y no adelanta la validación física (**G1–G7 siguen `pending`**). Nace de tres peticiones del gerente sobre la app **en uso**: que el diccionario se parezca a los de las apps de referencia, que el espacio para escribir la palabra sea **grande y claro sobre todo en móvil**, y que **cada sentido de la consulta tenga su color**; al cerrar la sesión pidió además que la Ayuda declare **qué compilación** está corriendo. **(A) El buscador es la vista, no un control más.** Antes era una fila de **40 px** con el botón al lado, compitiendo con el `h1`, con las pestañas de la pantalla y con el conmutador de sentido, que flotaba **encima** del campo sin relación visual con él. Ahora el campo vive dentro de una **tarjeta-buscador** (`rounded-2xl`, marco de 2 px, barra superior de color, anillo de foco del mismo color) y mide **48 px en móvil y 56 px en escritorio/tablet**, medido y no estimado; el icono de lupa va teñido del sentido activo, y el **conmutador EN→ES / ES→EN entra dentro del buscador** en dos pastillas a ancho completo —dentro no compite con las pestañas y se lee como lo que es, una propiedad de la consulta—. Sin consulta todavía, la vista ofrece **cuatro palabras de arranque por sentido** (que se marcan con su `lang`, porque la entrada es del otro idioma) y un botón de borrado (`X`) que **solo existe cuando hay texto**: nada de botones muertos en reposo. **(B) Un color por sentido, y el color es una convención declarada.** EN→ES va en **azul** y ES→EN en **fucsia**, con el relleno y el borde **derivados de la tinta** (`color-mix()` al 15 % y al 35 %), el mismo mecanismo que la rampa de niveles de V3.75.4: un consumidor nuevo no exige tocar un hex. La pareja es **complementaria** y **no pisa los colores de estado** —verde dominada, ámbar en curso, rojo débil—, que ya significan otra cosa. Y, **a diferencia de la rampa, no sigue el acento del perfil**: si el color del sentido cambiara con el acento, dos acentos afines podrían dejar azul y fucsia casi iguales y la señal se perdería; es una **leyenda del diccionario** (azul ↔ fucsia), no una preferencia. `directionClass()` (`utils/dictionaryDirection.ts`) devuelve el **nombre de una clase** (`.dir-en-es` / `.dir-es-en`), nunca una interpolación de Tailwind que el escaneo purgaría. **(C) Cuál manda: el buscador se tiñe del sentido ACTIVO, la tarjeta del sentido DE SU CONSULTA.** El resultado usa `entry.direction`, con una marca de dirección («English → Spanish») junto a la palabra, así que **conmutar el buscador después de buscar no repinta la tarjeta**: el resultado no puede mentir sobre en qué sentido se pidió. Dentro de la ficha el color se concentra en el **equivalente** —el bloque con más tinta y `text-lg font-semibold`, porque es la respuesta— y en la pestaña del ejemplo; el resto queda en neutro para que el color **informe** en vez de decorar. **(D) Un solo `h1` en `/diccionario`.** La vista de consulta traía su propia cabecera y la pantalla la suya: había **dos `h1`** en la misma página y el título repetido. `DictionaryLookup` recibe `showHeader` (**defecto `true`**, que es lo que necesitan las superficies que la montan suelta) y `DictionaryScreen` lo apaga; el ancho y el relleno de página ya los pone el contenedor de la pantalla, así que la vista dejó de aplicarlos por segunda vez. **(E) La Ayuda declara qué compilación está corriendo.** Nuevo `utils/buildInfo.ts`: la versión se lee del `package.json` **en tiempo de compilación** y viaja dentro del bundle —solo el campo `version`, el resto del fichero no entra— porque la Ayuda es una pantalla **sin red** y hasta ahora la única versión visible era la del **backend** (`GET /api/health` → `SystemStatus`), justo la que **no** responde cuando algo va raro. `check_release_consistency.py` mantiene ese número igual al de los otros seis orígenes, así que la versión de la compilación y la que declara el backend no pueden divergir en una release publicada. La tarjeta de vocabulario de la Ayuda explica además el **sentido** de la consulta y su color. **(F) Medido, no mirado.** El par de direcciones entra en el arnés de contraste (`--strict`): tinta de cada sentido sobre su **relleno compuesto** en los **2 temas** × **2 fondos**, con el porcentaje de mezcla **leído del CSS** en vez de supuesto —**5.76–8.02:1** frente al mínimo 4.5:1 (**480 pares + 6 guardas, 0 bloqueantes**)—, y gana dos guardas con dientes: `direccion-completa` (las dos tintas en los dos temas) y `direccion-clases-y-derivados` (relleno al 15 %, borde, las clases `.dir-*` y el marco `.dir-field`). Es la guarda que importa: si `directionClass()` devolviera una clase inexistente, **el buscador se quedaría sin color y ninguna otra prueba se enteraría**. En unitario, `dictionaryDirection.test.ts` fija el mapeo sentido→clase→clave i18n y `DictionaryLookup.test.tsx` exige que la pastilla activa lleve la clase de **su** sentido, que la tarjeta se tiña del sentido **con el que se buscó**, que el botón de borrado no exista en reposo y que pulsar un ejemplo rellene el campo y dispare **una** consulta; `HelpScreen.test.tsx` (nuevo) fija que la versión de la compilación se ve junto al autor. La revisión visual se hizo con un spec **temporal** en los **tres breakpoints** —alto de campo, conmutador con dos colores computados distintos, cero desbordamiento horizontal— y **se borró después**. **(G) Verificación.** `tsc --noEmit` limpio, `vitest run` **819/819** (95 ficheros), `npm run build` correcto (el bundle publica `3.75.8`), i18n `--strict` **1515 claves / 0 huérfanas**, contraste `--strict` **480 pares + 6 guardas / 0 bloqueantes**, `ruff` limpio, `pytest` **2909/2909**, `check_release_consistency` OK en los **6 orígenes** (`3.75.8`), `validation_gate.py auto` **10/10** y el spec visual de móvil que abre la Ayuda (`mobile.spec.ts`, 6 casos) en verde. **Honestidad.** (i) **El color no añade información, evita una confusión:** el sentido ya lo decía el rótulo y el color solo lo adelanta; quien no distinga azul de fucsia sigue teniendo el rótulo. (ii) **La pareja no es configurable a propósito:** ofrecerla como preferencia la convertiría en algo que puede dejar de coincidir entre lo que el alumno cree y lo que la app dice. (iii) **Dos compilaciones de la misma versión son la misma compilación declarada:** la Ayuda muestra la versión, **no** la fecha ni el `hash` del build, así que no distingue un `dist` recompilado del original. (iv) **El diccionario se queda sin spec visual permanente:** la medición se hizo con un spec temporal que se borra, de modo que una regresión de layout en esa pantalla solo la vería otra revisión manual. (v) **El `dist` no se versiona** (`.gitignore`): hay que recompilar tras desplegar. (vi) El **P0 de identidad sigue entero** —esta release no añade ni una ruta ni un endpoint— y **los 7 gates siguen en `pending`**. Ver `release-notes-v3.75.8.md`.

- ✅ **V3.75.7 — El icono del desplegable y el texto que no queda debajo del botón + cierre del P1 del banco heredado (2026-09-20)** (**Versión estable `3.75.7`**, app `3.75.2 → 3.75.7`). **Release de PRODUCTO (minor) que publica CINCO ITERACIONES juntas** —V3.75.3 · V3.75.4 · V3.75.5 · V3.75.6 · V3.75.7, las cinco registradas una por una en este mismo `PLAN.md` y en `CHANGELOG.md`— **bajo una sola etiqueta `v3.75.7`**: es la única que lleva tag. **SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el currículum (`CURRICULUM_VERSION` sigue `1.3.1`) ni las evaluaciones, y SIN tocar `LISTENING_BANK_VERSION`.** No añade ni retira un gate y no adelanta la validación física (**G1–G7 siguen `pending`**). La pregunta «¿por qué un número y cinco iteraciones?» se responde abajo, en **(F)**. **(A) El icono del desplegable es un contrato, no un adorno.** `InfoDisclosure` gana `content: "info" | "options"` con **defecto `"info"`**: un panel nuevo que no declare nada no puede prometer opciones que no tiene. El disparador pinta **(i)** cuando el panel solo explica y **(...)** cuando además trae opciones, y la etiqueta accesible acompaña al icono (`common.moreInfo` / `common.moreOptions` = «Opciones e información»). Se revisaron **11 disparadores** en toda la app y ninguno quedó sin clasificar: **8** pasan a (i) (notas de ruta de listening y notas de los paneles de nivel de las seis destrezas —speaking tiene dos—), **1** conserva (...) porque de verdad cuelga del `VoicePicker` (rutas de quiz), y **3** quedan como estaban, declarados (el «...» de la tarjeta de audio, que ya era «...» porque trae velocidad + voces + repetición; el desplegable de texto «Cómo funcionan las rutas», que ya era (i) con etiqueta visible; y el botón de los propios paneles). **(B) El texto ya no queda debajo del botón.** En la variante de esquina el disparador flota sobre la `Card` y el panel nace justo debajo (era su primer hijo), así que su primera línea salía **tapada** por el botón que acababa de abrirla: medido, el párrafo «A real CEFR B1 means…» quedaba dentro del rectángulo del botón. El panel reserva ahora la **columna** del disparador (`py-2 pl-3 pr-12` = 36 px del botón + 12 px de aire) y **no altura**, que habría dejado un hueco muerto en los paneles que no llegan al borde superior. La composición se hace explícita frente a `px-3 py-2` para **no depender del orden** en que Tailwind emita `px-*` y `pr-*`. **(C) El P1 que destapó la auditoría externa: el banco heredado seguía sirviendo la tarjeta de dictado.** `test_production_items_do_not_carry_multiple_choice_options` filtraba ids `c` (solo el corpus) y `PRODUCTION_SKILLS` estaba duplicado en tres sitios, así que **`l18` (`dictation`) y `l19` (`shadowing`) seguían en el banco heredado con opciones**: la RUTA B1 continuaba mostrando «Enviar dictado» —exactamente el defecto que V3.75.6 decía haber cerrado— y verificado por comando (`B1 prod [('l18','dictation',4), ('l19','shadowing',4)]`) frente a `corpus prod con opciones []`. Cerrado en la causa raíz: la lista se declara **una sola vez** (`backend/services/listening_flow.py::PRODUCTION_SKILLS`, consumida por `build_item_flow`), el barrido es sobre **todo** `QUESTION_BANK`, y `l18` → `numbers` y `l19` → `phrase_recognition`. **(D) El test que no mordía, arreglado en los dos sentidos.** La guarda pasa a barrer el banco entero **y** se le añade un control positivo (`test_production_guard_bites_on_a_synthetic_offender`): hoy no queda ningún ítem de producción, así que su barrido es vacío y podría pasar **por estar roto**. Además, la suite deja de apoyarse en ítems concretos del banco: nuevo fixture `production_items` en `tests/conftest.py` que construye ítems de producción **sintéticos** (copia de un ítem real con el `skill` cambiado), y migran a él `test_listening_production.py`, `test_word_breakdown_v329.py`, `test_listening_shadowing2_v328.py` y `test_listening_attempts_v327.py`. `test_bank_covers_every_subskill` → **`test_bank_covers_every_receptive_subskill`**: el banco **no puede** contener producción (el esquema `ListeningAsset` exige `question` + `options` + `answer_index` en todos), así que la ausencia se exige **exacta** (todas las receptivas cubiertas, y exactamente las de `PRODUCTION_SKILLS` fuera). **(E) Dos P3 y un launcher que se puede probar.** (1) `domain/listening.py::get_audio` devuelve la voz **realmente usada** y `routers/listening.py` la publica en **`X-TTS-Voice`**: el frontend ya no puede creer que oyó dos acentos cuando, por falta de voz instalada, sonó la misma dos veces. (2) `launcher/config_store.py::save_config` escribe a un temporal y hace `os.replace`, así que un corte a mitad ya no deja `config.json` a medias. (3) La lógica de LAN se centraliza en `launcher/core.py` (`apply_stored_lan_config`, `toggle_lan_config`) para que la decisión que cambia la **frontera de red** tenga test fuera de `tkinter`, que no se puede probar sin pantalla. **(F) Por qué cinco iteraciones y una etiqueta.** Las cinco nacieron del uso real de la app por el gerente, se documentaron **una a una** en `docs/audit/PARKED.md` con su decisión y su precio, y los comentarios de código las citan por su nombre (`V3.75.5`, `V3.75.6`, `V3.75.7`). Publicarlas con un tag por iteración exigía reconstruir cinco releases con cinco ciclos de verificación sobre trabajo ya hecho; publicarlas con **una sola entrada** habría dejado cuatro etiquetas citadas en código sin nada que las resolviera. Se elige el punto medio **decidido con el gerente**: **cinco entradas** en `CHANGELOG.md` y `PLAN.md` —para que cada etiqueta siga siendo localizable— y **una sola etiqueta `v3.75.7`**, que es la que la auditoría externa ancla. Queda declarado que `V3.75.3`–`V3.75.6` **no tienen tag propio**. **(G) Verificación.** Backend **2909 passed** (0 skipped; el recuento sube por los tests nuevos y por cinco suites que se reformularon **en sitio**, no se duplicaron), `ruff` limpio, frontend vitest **806/806** (93 ficheros), `tsc --noEmit` limpio, i18n `--strict` **1512 claves / 0 huérfanas**, contraste `--strict` **472 pares + 4 guardas / 0 bloqueantes**, `check_release_consistency` en los **6 orígenes** (`3.75.7`), `validation_gate.py auto` **10/10** y `dist` recompilado. La guarda del texto tapado **mordió de verdad**: revirtiendo `pr-12` a `px-3`, el spec visual vuelve a fallar con `coveredNotes: ["p|A real CEFR B1 means hundreds of known w…"]`; y el spec recorre la **RUTA B1** en desktop/tablet/móvil (390/768/1280) comprobando que las opciones están y la tarjeta de producción no, que el «…» ofrece las tres composiciones con la primera activa, y —leyendo el cuerpo real del `POST /api/tts`— que el texto compuesto es texto + pregunta + opciones + clave. **Honestidad.** (i) **El provenance del banco no distingue el antes del después:** `LISTENING_BANK_VERSION` sigue en **`7.0.0`** a propósito, porque además de identificar el banco **nombra la caché de audio** y ningún `script` ni `audio_id` cambió; subirlo habría re-sintetizado el banco entero por cero audio distinto. El hueco (un mismo `bank_version` para dos contenidos servidos distintos) queda **declarado** y es candidato a hallazgo del auditor, no una omisión: la política del proyecto sí bumpea provenance cuando el coste es cero (así se hizo con `CURRICULUM_VERSION` en V3.75.1), y aquí el coste no lo es. (ii) **Hoy no queda ningún ítem de dictado autorado:** hay endpoint, flujo, UI y tests, pero el esquema del corpus exige `options` + `answer_index` y no puede representar un dictado; es deuda de **contenido**, no de código. (iii) **`l18`/`l19` cambian de etiqueta, no de contenido:** su `script`, sus opciones y su audio son los mismos; lo que cambia es por qué flujo se sirven y, por tanto, cómo se ven en B1. (iv) El **acento es simulado** (Piper sintetiza; una voz «británica» no es un hablante británico). (v) Los desplegables de los paneles de nivel son `inline` —botón y panel apilados en columna— así que el solapamiento es imposible **por construcción** y están cubiertos por unitario, pero **no** se navegó a las seis pantallas para medir su geometría real como sí se hizo en listening: si alguna se pasara a `corner`, la guarda visual no la cubre. (vi) La suite visual **no aguanta el paralelismo** del arnés en Windows (medida con `--workers=1`). (vii) **El P0 de identidad sigue entero** y esta tanda no lo toca; V3.75.3 le añadió incluso una vía más para arrancar expuesto (la preferencia LAN persistida). (viii) **Los 7 gates siguen en `pending`** y la identidad sellada en el kit sigue siendo la del pre-vuelo (`3.73.6` → `13cc30b`), que es historia y no se reescribe. Ver `release-notes-v3.75.7.md`.

- ✅ **V3.75.6 — STOP en la repetición, lectura corta y la RUTA B1 que no se parecía a las demás (2026-09-20)** (**Versión estable `3.75.6`**, app `3.75.5 → 3.75.6`; **sin tag propio**, se publica dentro de `v3.75.7`). **Release de PRODUCTO (patch): SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el currículum y SIN tocar `LISTENING_BANK_VERSION`, pero SÍ toca contenido de listening** (`curriculum/listening_corpus.json` `3.0.0 → 3.0.1`), porque el defecto estaba en una etiqueta mal puesta y no en el flujo. **(A) La RUTA B1 no era un caso especial.** `c071` y `c084` —los **dos únicos** ítems del corpus (490) etiquetados `dictation`/`shadowing`— estaban **autorados como pregunta de opción múltiple** (`question` + 4 `options` + `answer_index`), y el flujo se elige por `skill`, así que la app pedía **escribir a mano** una frase cuyo contenido autorado era «Which time did you hear?» con opciones: la tarjeta que veía el alumno no se parecía a las otras cinco rutas y el contenido autorado se tiraba. Corregido **en el contenido**: `c071` → `numbers`, `c084` → `phrase_recognition`. B1 vuelve a recibir lo mismo que las demás y **recupera dos ítems servibles**. **(B) STOP en la repetición, y una sola locución a la vez.** `api/voz.ts` gana un registro de módulo con la locución en curso: `stopSpeaking()` pausa el audio **y** cancela la síntesis si aún no había sonado, y la promesa de `speak()` **resuelve** —parar no es un error—, así el botón apaga su spinner sin tratar el corte como fallo. Empezar otra locución corta la anterior (antes se solapaban). El STOP solo existe mientras suena y vive junto a los botones A/B. **(C) `replay_scope`: tres composiciones, no dos.** Un ítem tiene cuatro piezas —**texto** (el `script`, lo que suena), **pregunta**, **opciones** y **respuesta correcta**— y de ahí salen `"item"` (**defecto**: texto + pregunta + respuesta), `"withOptions"` (texto + pregunta + opciones + respuesta) y `"correct"` (pregunta + respuesta). Es una clave más del mismo `PUT /api/settings`, así que no hay backend nuevo. El valor histórico `"all"` se normaliza a `"withOptions"` (quien lo había elegido de forma explícita no pierde su elección) y ausente o desconocido cae a `"item"`. **(D) Una sola función decide el texto** (`utils/voices.ts::buildReplayText`), con salvaguardas: descarta piezas ausentes en vez de inventarlas, **no repite** el ítem cuando el script *es* la pregunta (habitual en A1) y, si el alcance elegido dejara la lectura vacía, cae a la composición completa —nunca una repetición muda—. No puede **filtrar la respuesta antes de tiempo**: `correct_index` solo lo aportan los sitios que ya han respondido, y hasta entonces el altavoz de repetición ni existe. Bajo los botones se describe **la composición activa** («Se lee: …») en vez de un párrafo que explique las tres: las etiquetas se parecen («el ítem completo» / «el ítem más las opciones») y la diferencia está en las **piezas**, no en las palabras. **(E) El test deja de declarar y pasa a medir en pantalla.** `frontend/tests/visual/listeningReplayStop.spec.ts` (mock-based, 3 breakpoints) navega a la RUTA B1 y comprueba que las opciones están y la tarjeta de producción no, que el «…» ofrece las tres lecturas con la primera activa, y que tras responder aparecen A/B y el STOP; además **lee el cuerpo real del `POST /api/tts`** y exige que el texto compuesto sea texto + pregunta + opciones + clave. Medido: STOP **32×32** dentro del viewport y **sin desbordamiento horizontal** (390/390 · 768/768 · 1280/1280). Que el **banco** sirva una pregunta receptiva en B1 lo fija `test_listening_corpus.py`: la suite visual mide la UI, no el contenido. **(F) Verificación:** `tsc` limpio, `vitest run` **805/805**, i18n `--strict` **1511 claves / 0 huérfanas**, contraste **472 + 4 / 0 bloqueantes**, `ruff` limpio, `pytest` **2907/2907**, `playwright --workers=1` **41/41** y métricas del corpus regeneradas. **Honestidad.** (i) **Hoy no queda ningún ítem de dictado**: el endpoint y el flujo siguen vivos y probados, pero el esquema exige `options` + `answer_index` y no puede representar uno; quien quiera dictado real tendrá que autorarlo con un esquema que lo represente. El **shadowing** no desaparece de la práctica: sigue siendo el paso opcional del micro-flujo receptivo. (ii) **Cambiar el defecto es un cambio visible:** hasta ahora la única lectura era la larga y quien no toque nada oirá menos piezas. (iii) **El corpus en marcha no se recarga solo** (el backend lee el banco al importar) ni la UI compilada en `dist`: hay que reiniciar y recompilar. (iv) **`docs/LISTENING_ENGINE_4.0.md` (fechado 2026-09-09)** citaba `c071`/`c084` como evidencia de dictado y shadowing: es una especificación fechada y no se reescribe, pero se le añade la nota de corrección para no dejar una cita falsa en un documento que se sigue leyendo. (v) La suite visual **no aguanta el paralelismo** del arnés en Windows. Ver `release-notes-v3.75.7.md`.

- ✅ **V3.75.5 — Voz configurable y dos acentos en listening y rutas de quiz (2026-09-20)** (**Versión estable `3.75.5`**, app `3.75.4 → 3.75.5`; **sin tag propio**, se publica dentro de `v3.75.7`). **Release de PRODUCTO (minor): SIN migración de BD, SIN contenido nuevo y SIN endpoints nuevos.** Cero backend nuevo para la preferencia: la voz A es `tts_voice` y la B es `tts_voice_alt`, dos claves del mismo `PUT /api/settings`, y la caché de audio ya estaba separada por voz, así que el segundo acento es **una caché más**, no un motor nuevo. **(A) El «…» deja de ser un cartel y pasa a configurar.** Antes solo leía «Voz sintética local (TTS) · <voz> · <wpm>» y no había **ningún** camino a la configuración desde la práctica. Ahora abre el bloque `VoicePicker`: fila **Voz A** (la del perfil), fila **Voz B** (segundo acento) y dos chips **«Probar A» / «Probar B»** que reproducen *ese* ítem para comparar antes de decidir. El mismo bloque, tal cual, cuelga del «…» de las rutas de quiz: **una sola idea de acento en toda la app**. **(B) Se puede pedir voz, y solo una instalada.** `TTSRequest.voice` y el query param `voice` de `/api/listening/audio/{id}` viajan hasta `tts.pick_requested_voice`, que **solo** acepta una voz instalada y del idioma pedido; si no pasa el filtro **se ignora en silencio** y manda `resolve_voice(prefs)`. No es laxitud: es que la UI no puede romperse por una preferencia vieja (ni recibir un 400 por ella) y, sobre todo, **el id de voz entra en rutas de disco** (`PIPER_DIR / "<voz>.onnx"` y `data/listening/{banco}/{voz}/…`), así que aceptarlo sin validar sería *path traversal* por construcción. `X-TTS-Voice` / `X-TTS-Degraded` siguen declarando lo que **realmente** sonó, no lo que se pidió: la UI puede ser honesta cuando degrada. **(C) La B se sugiere sola y nunca cruza idiomas.** `utils/voices.ts::suggestAltVoice` propone la primera voz del **mismo idioma y otra locale** (US → GB y al revés), de modo que «dos acentos» funciona recién instalada la app sin obligar a configurar nada; el selector solo ofrece voces del idioma de la voz A, porque mezclar idiomas haría que la B leyera el ítem en otro idioma. **(D) Un store de módulo, no un contexto.** `hooks/useVoiceChoice.ts` (patrón de `useVoiceDownload`, `useSyncExternalStore`) hace **una sola** lectura de `GET /api/voices` + `GET /api/settings` para toda la app aunque haya varios altavoces en pantalla, y **vacía el estado al cambiar de perfil** (la voz de un alumno no puede quedarse a la vista de otro). Configuración → Voces escribe `tts_voice` por su cuenta, así que el panel **relee** el store al elegir voz y al instalar una nueva (`refreshVoiceChoice`): sin eso la voz A de la práctica quedaba desincronizada. **(E) El altavoz de después de responder lee el ítem entero** en voz A o B, y con una sola voz instalada **solo** aparece A: nunca un botón que no puede sonar. **PLAY sigue sonando en voz A**: la escucha de estudio es una sola y el segundo acento vive donde el alumno lo pide, que es después de responder. `ListenButton` no cambia cuando no se le pide acento (40 y pico usos previos y sus tests intactos); donde el altavoz lee un **ítem** se usa `ItemReplayButton`, y donde lee texto arbitrario del alumno (traductor), el de siempre. **(F) El mismo control, allí donde se escucha.** Se sustituye el altavoz mudo por el consciente de la voz en los paneles de nivel de listening/vocabulario/gramática/conversación/pronunciación/speaking, en los escenarios de pronunciation y speaking, en el drill de vocabulario, en el diccionario (palabra y ejemplo) y en la evaluación de speaking. **(G) Las rutas de quiz adoptan la convención visual de V3.75.4:** las seis rutas **de dos en dos** (A1·A2 / B1·B2 / C1·C2) con celda horizontal (anillo + datos), **«Auto» en fila propia** a ancho completo —es la opción que *no* elige ruta y no debe competir con las seis que sí—, **rampa de color**, **separadores finos** y el «Saltar» discreto en la **cabecera** (63×32 px medidos), visible **también dentro de una sesión** y oculto solo mientras se evalúa o cuando ya hay resultado: retirarlo durante las sesiones habría sido una regresión silenciosa, porque es la única salida de un ítem sin registrar evidencia. **(H) Medido, no estimado** (playwright, 3 viewports, mock determinista de las rutas de gramática): 6 celdas en **3 filas de 2** en 390/768/1280, sin desbordamiento horizontal (`scrollWidth == clientWidth`), celda de **154×66** px en móvil y **335×70** en tablet/escritorio, «Auto» a ancho de tarjeta (316/678 px), y los dos altavoces (A y B) presentes tras responder. **(I) Verificación:** `tsc` limpio, `vitest run` **777/777**, `npm run build` correcto, contraste **472 + 4 / 0 bloqueantes**, i18n **1504 claves / 0 huérfanas**, `ruff` limpio, `pytest` **2906/2906** y los 6 specs visuales de rutas (desktop) en verde. **Honestidad.** (i) **La primera reproducción en voz B cuesta unos segundos**: sintetizar y alinear (ASR) ese WAV es trabajo real la primera vez que se pide cada ítem × voz × variante; después queda cacheado. El chip muestra su spinner y no se queda mudo, pero no es instantáneo y no se disimula. (ii) **La caché crece** (~100 KB por ítem × voz × variante): es disco local y se puede borrar sin romper nada, pero no se recolecta sola. (iii) **El acento sigue siendo simulado.** (iv) El barrido visual completo sigue sin hacerse: se midió la rejilla de rutas y la presencia del control de voz, **no** cada pantalla donde se sustituyó el altavoz. Ver `release-notes-v3.75.7.md`.

- ✅ **V3.75.4 — Listening pulido y rampa de niveles configurable (2026-09-20)** (**Versión estable `3.75.4`**, app `3.75.3 → 3.75.4`; **sin tag propio**, se publica dentro de `v3.75.7`). **Release de PRODUCTO (minor): SIN migración de BD, SIN backend nuevo y SIN contenido nuevo.** Su objeto es que la pantalla de listening se lea de un vistazo y que **el color de nivel signifique algo**, con el color medido y no decidido a ojo. **(A) El «Saltar» del pie se conserva, discreto y reubicado.** Era `listening.skip` y llamaba a `load()`: **pide otro ítem sin responder**, la única salida de una pregunta sin registrar evidencia (audio que no suena, ítem incomprensible). No se elimina (habría quitado la única salida), sino que sale del pie —donde competía con las opciones— y pasa a la cabecera nueva como botón fantasma pequeño (`listening.anotherItem`; la clave anterior se renombra para no dejar huérfana). No se confunde con el otro «Saltar» de la tarjeta de shadowing (`listening.flow.skipStage`), que declina el paso **opcional** que el backend marca con `allow_skip`. **(B) El color de nivel pasa de 3 tramos a 7 pasos con un solo mecanismo.** Antes `cefrTone` devolvía `basic|intermediate|advanced`, así que A1 y A2 (o B1 y B2) eran del **mismo** color y el color no decía nada. Ahora `cefrLevelKey`/`levelClass` (Pre-A1 → C2 + «sin dato») son la **única puerta** y las clases **estáticas** `.lv-*` de `styles/legacy.css` la **única pintura**: sin clases Tailwind interpoladas (el escaneo las purgaría) y sin `inline style`. Se declaran **fuera de capa** a propósito, para que una utilidad de Tailwind de la misma línea no borre el color del nivel. **(C) Fallo previo que se arregla de paso.** `LearningProfile.tsx` llamaba `cefrTone(profile.estimated_bands[skill])` con un **número** (escala 0–6), así que caía **siempre** en `basic` y las filas del perfil se pintaban iguales: `bandToLevelKey(numeric)` recupera el tramo, declarado como **aproximación de color**, no como medida de dominio. **(D) La rampa es elegible por usuario (Ajustes > Apariencia), sin backend nuevo.** Tres esquemas —**Semáforo** (por defecto, reproduce los colores que ya había), **Espectro** y **Monocromo** (7 intensidades del acento, cero hexes propios)— con vista previa en el propio diálogo. Se aplica con `data-levels` en `<html>` (igual que `data-theme`/`data-accent`/`data-density`), se persiste por perfil con la clave `level_scheme` en el `PUT /api/settings` que ya existía y `localStorage` como copia para que un backend caído no pierda la preferencia. El relleno y el borde se **derivan** de la tinta con `color-mix()`, así que un esquema nuevo son 7 hexes y **no 21**. **(E) El gate del color es la medición, no el gusto.** `contrast_audit.mjs` mide cada paso sobre su **relleno compuesto** (sobre `--color-surface` y sobre `--color-bg`) en los **3 esquemas × 2 temas** —y en Monocromo, por los **7 acentos**—, con guardas de que la rampa esté completa: **472 pares + 4 guardas, 0 bloqueantes** en `--strict`. Los hexes se ajustaron hasta AA; el precio es que la paleta es **tributaria del contraste**, no al revés. **(F) El selector de rutas del listening se agrupa de dos en dos.** Las seis rutas (A1·A2 / B1·B2 / C1·C2) pasan a **dos columnas** en todos los anchos —celda horizontal, anillo a la izquierda y datos a la derecha— y **«Auto»** sale de la rejilla a una **fila propia a ancho completo**, después de C2: es la opción que **no** elige ruta y no debía competir con las seis que sí la eligen. Entre el resumen y la rejilla, y entre esta y el panel de nivel, quedan **separadores finos** (`border-border/60`). **(G) Medido, no estimado:** con el arnés en 3 viewports, las seis celdas ocupan **3 filas de 2** y no hay desbordamiento horizontal (`scrollWidth == clientWidth` en 390/768/1280); en móvil cada celda mide **158 px** de ancho y entre **66 y 107 px** de alto, y «Auto» ocupa el ancho de la tarjeta. **Honestidad.** (i) **Alcance de la medición:** mide la tinta sobre el relleno compuesto en esos dos fondos, **no** cada sitio donde se dibuja una insignia; y que un nivel **tenga color no implica que el alumno domine ese nivel**. (ii) **El barrido visual completo sigue sin hacerse:** de esta release se ejecutó **solo la parte de listening** (`routeExtraReview` y la medición geométrica de la rejilla), y el barrido de la rampa en toda la app y de **Ajustes** queda pendiente. (iii) **`cefrTone` sobrevive en documentos históricos** (briefs y actas de `agentes/`, `RELEVO.md`, `CHANGELOG.md`): son documentos fechados de lo que se decidió entonces y **no se reescriben**; el código ya no tiene ninguna referencia. (iv) **El P0 de identidad sigue entero** y este incremento no lo toca (ninguna ruta nueva, ningún endpoint nuevo). Ver `release-notes-v3.75.7.md`.

- ✅ **V3.75.3 — Preferencias del launcher, listening sin fricción y Análisis global (2026-09-20)** (**Versión estable `3.75.3`**, app `3.75.2 → 3.75.3`; **sin tag propio**, se publica dentro de `v3.75.7`). **Release de PRODUCTO (minor): SIN migración de BD, SIN contenido nuevo y SIN endpoints nuevos.** Nace de tres peticiones del gerente sobre la app **en uso**: que el launcher **recuerde** la última configuración (en concreto «ACTIVADA RED LOCAL»), que la práctica de listening de móvil no gaste un paso entero en «He escuchado — responder», y que el **Análisis** esté en la cabecera junto al usuario mostrando la evolución del alumno **en su conjunto**, en vez de un panel flotante dentro del ejercicio. **(A) La preferencia de red se persiste** en `launcher/config.json` (`launcher/config_store.py`), se lee **al arrancar** antes de pintar la interfaz y se guarda **en cada cambio**. Responde a la pregunta abierta nº 1 de `docs/audit/PLAN-P0-IDENTIDAD.md` y **revierte §5.8** del mismo documento: el modo LAN pasa de decisión de **sesión** a decisión de **instalación**. **Lo que esto reabre, dicho sin adornos:** un equipo con `{"lan": true}` arranca con uvicorn en `0.0.0.0` y aceptando orígenes de red privada **sin que nadie lo declare en esa sesión**, y el P0 de identidad sigue abierto (`GET/POST /api/users` siguen sin credencial). **Mitigación (no lo elimina):** el modo es **visible** desde el primer pintado (fila LAN con el enlace y botón «Desactivar red local»), el fichero es local y se ignora en git, el **default es cerrado**, y un valor editado a mano que no sea el booleano `True` (`1`, `"sí"`) se lee como **cerrado**. Candados: `launcher/tests/test_config_store.py` y las cuatro pruebas nuevas de `launcher/tests/test_lan_mode.py` (aplicar al arrancar, sin preferencia, valor no booleano, y preferencia **por encima** del entorno heredado). **(B) Listening sin el paso «He escuchado — responder».** La tarjeta `while1` desaparece y la señal de «he escuchado» es **pulsar PLAY**: `play()` avanza la etapa al **empezar** el audio (en el TTS, al lanzarlo; `speakWithVoice` resuelve al terminar), y en el `catch` **no** avanza, para que un fallo de audio deje el ejercicio donde estaba. **Efecto deliberado:** sin pulsar PLAY no se ven las opciones. Se retiran tres claves i18n huérfanas; `microFlow.ts` no se toca. **(C) Compactación móvil de la misma pantalla:** contenedor, tarjeta de audio, botón PLAY, waveform y una fila de controles A/B que se oculta por debajo de `sm`. Se **conserva** el deslizador de posición (una línea, sirve para releer un tramo) y todos los objetivos táctiles (`min-h-10`). **(D) El Análisis se va a la cabecera y a su propia ruta.** `features/analysis/AnalysisScreen.tsx` vive en `/analisis` (`ANALYSIS_PATH`) y se abre con un botón en el bloque `ml-auto` del `Header`. Es **destino auxiliar**: **no** lleva píldora en `Navigation` (por eso no toca `ROUTES` ni rompe el test de las 5 píldoras). Sintetiza posición, actividad real con su agrupación temporal, tríada, destrezas y escalera CEFR reutilizando endpoints que **ya existían**, sin crear ninguno. **(E) Se retira el panel flotante:** `components/AnalysisPanel.tsx` **borrado**, fuera las reglas CSS muertas (`.pane--insights`, `.insights-header`, `.insights-toggle`, `workspace--learn`, `chat.resizeInsights`, …) y el asa derecha del workspace. El panel se había quedado sin analítica propia en V3.1 y solo se encontraba **dentro** del ejercicio. La calidad del tutor no se pierde: la recibe la pantalla nueva desde los turns de la sesión en curso. **Honestidad.** (i) **El P0 de identidad, entero**, y ahora con una vía más para arrancar expuesto: la preferencia persistida; la Fase 3 (credencial o emparejamiento) **no cambia de prioridad** por esta decisión. (ii) **El barrido visual de `/#/analisis` y del listening compacto no se ha ejecutado** (Playwright en Windows es frágil); `resize.spec.ts` se reescribió sobre el asa del **sidebar** —la que sobrevive— porque probaba la derecha, que ya no existe. (iii) **`layout.rightWidth` sigue existiendo y persistiéndose** sin consumidor: el asa del panel se retiró, pero el contrato de layout tiene sus propios tests y no se poda en esta release para no arrastrar cambios al `useChat`. Es **deuda de forma, no una fuga**. Ver `release-notes-v3.75.7.md`.

- ✅ **V3.75.2 — Release documental: la pausa pedagógica pre-baseline se publica y se deja auditable desde GitHub (2026-09-19)** (**Versión estable `3.75.2`**, app `3.75.1 → 3.75.2`). **Release DOCUMENTAL y de INSTRUMENTO: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN una sola línea de código de producto.** No añade ni retira un gate y no adelanta la validación física (**G1–G7 siguen en `pending`**). **(A) El motivo.** Tras cerrar el P0 posicional, el gerente decide **no publicar una V3.75.2 de corrección** para el sesgo de longitud: seguir tocando el banco sin entender el patrón completo sería contraproducente. Se abre una **pausa de medición** sobre el baseline congelado **v3.75.1 = `7962d57`**, y esa medición se **publica con ancla estable** para que un auditor externo pueda clonarla: sin commit y sin tag, GitHub no la ve. **(B) El instrumento (solo lectura).** `backend/scripts/audit_dossier.py` gana dos subcomandos y una corrección: `item-form` (nº de opciones, posición y longitud de la correcta, por banco, nivel y grupo de `k`, con posiciones muertas) y `distractor-signals` (tres heurísticas declaradas —eco de palabra clave del enunciado, outlier de longitud y literal de cantidad— más una muestra determinista), y `mc-bias` **separa exámenes de placement**: hasta ahora el grupo llamado «exámenes level/placement» **solo contaba los 22 exámenes** y el **placement (24 ítems) no aparecía** en esa medición, así que el «63,6 % en posición 0» de V3.75.1 es de **exámenes**, no del conjunto. `mc_banks()` pasa a ser la fuente única de los tres ejes. **Nada de esto toca contenido ni motor**; el par `.md`/`.json` se emite en `docs/audit/generated/`. **(C) La ejecución: cinco ejes en paralelo.** Cada eje produjo su propio dossier con la regla de evidencia `[D]` declarado / `[A]` árbol / `[R]` reproducible: **AH** · longitud (`AH-PSICO-LONGITUD.md`) · **AI** · instrumentos de evaluación (`AI-PSICO-ASSESSMENTS.md`) · **AJ** · forma del banco (`AJ-PSICO-FORMA.md`) · **AK** · distractores (`AK-PSICO-DISTRACTORES.md`) · **AL** · patrón por nivel (`AL-PSICO-NIVELES.md`), con la síntesis **AM** (`AM-SINTESIS-PSICOMETRIA-V3751.md`). **(D) El resultado: 0 P0 · 4 P1 · 12 P2 · 9 P3 (25 hallazgos, 5 problemas reales tras deduplicar los cruces).** `V3.75.1` sigue siendo la base correcta para sellar, y esta auditoría es su **acta de deuda de forma**: **(1)** sesgo de longitud real, **concentrado por lote** (corpus C1/C2 **80 %**, B1 checks 50,9 %, C2 checks 56,1 %, placement 50 %; los checks globales 39,1 %, +6,0 pp sobre el azar) y **no monótono por nivel**, lo que `CEFR-REFERENCE.md` no define como banda → es **deriva de autoría**, no incumplimiento CEFR; **(2)** los **instrumentos de evaluación conservan sus dos sesgos** porque V3.75.1 no los tocó: en los exámenes la **posición 2 está muerta** (0/22) y la 0 concentra el 63,6 %, en el placement la **posición 1 concentra 17/24 = 70,8 %** y la 2 aparece 1 vez; el atajo es **latente** (ningún componente consume el placement) y **contenido por umbrales** en el examen; **(3)** la forma del banco es **heterogénea por residuo**: 10 checks de `k=4` que son un accidente de autoría en A1 listening, corpus entero `k=4` y checks/exámenes `k=3`, sin ningún `k=2`; **(4)** **dos de las tres señales del instrumento subestiman** lo que dicen medir (`quantity_literal` dispara 1/904 y `prompt_keyword_echo` solo cuenta el eco exclusivo), declarado para que nadie lea un `0 %` como «sin problema»; **(5)** **el invariante de V3.75.1 aguanta también por nivel** (ninguna posición muerta), pero es **emergente**, no blindado. **(E) El ancla para la auditoría externa: el punto de entrada estaba caduco.** `agentes/auditoria-total-externa-v375.md` declara auditar `v3.75.0` y su invariante (`git diff --stat v3.75.0..main -- backend frontend launcher scripts` **vacío**) **ya no se cumplía**, porque V3.75.1 tocó `backend/`: seguirlo al pie de la letra producía un **P0 falso en el primer comando del auditor** (la regresión que V3.73.3 y V3.73.6 documentan y que V3.73.5 declaró cerrada). Se publica `agentes/auditoria-total-externa-v3751.md`, anclado a **`v3.75.1`** y **verificado por comando** —sin SHA ni run fijados a mano— con el **invariante honesto** de esta release: **producto y contenido sin cambios**, y el diff real declarado como **lista cerrada** (instrumento, test, artefactos, dossiers y documentación de release). **(F) Lo que NO hace esta pausa.** No corrige nada, **no añade candados que fijen el defecto** (los invariantes de forma se diseñan **con** la corrección) y **no decide** la política de forma ni el arreglo de `assessments.json`: **propone la fase**, no el cambio. **(G) Verificación.** Backend **2900 passed** (2882 + **18** del `test_psy_item_form_v3751.py`), `ruff` limpio, `check_release_consistency` en los **6 orígenes** (`3.75.2`) y el diff de producto **cero** comprobado por comando. Frontend, launcher y CI **sin cambios de producto**: ninguno consume el orden de las opciones ni el instrumento. **Honestidad.** (i) Mide **tasa de acierto explotable**, no aprendizaje: un ítem con la correcta más larga no es por sí solo un ítem malo. (ii) **No mide** la plausibilidad semántica de un distractor (exigiría hablantes) ni la **tasa real** del atajo (el motor guarda acierto, no posición marcada: la estrategia se **simula**). (iii) Los instrumentos de evaluación son **46 ítems, no 904** (placement 17/24 → IC95 % ≈ [52,6 %, 89,0 %]): no se generalizan con la confianza de los 368. (iv) El P0 de V3.75.1 **no se reabre**. (v) **Esta release no arregla nada del producto**: publica mediciones y repara un ancla documental. Ver `release-notes-v3.75.2.md`.

- ✅ **V3.75.1 — Cierre del P0 del sesgo posicional de los checks del currículum (2026-09-19)** (**Versión estable `3.75.1`**, app `3.75.0 → 3.75.1`). **Release de PARCHE y de CONTENIDO: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco y SIN tocar las evaluaciones (`assessments.json`), pero SÍ toca el currículum (`CURRICULUM_VERSION` 1.3.0 → 1.3.1).** No añade ni retira un gate y no adelanta la validación física (G1–G7 siguen `pending`). Cierra el **único P0** que quedaba abierto del motor pedagógico, medido por la auditoría de V3.70 en `docs/audit/AA-PED-CONTENIDO-CEFR.md`: **329 de 368 checks (89,4 %) tenían la respuesta correcta en la posición 0**, así que marcar **siempre la primera opción** acertaba casi 9 de cada 10 sin leer el enunciado. **(A) El problema, con su alcance exacto.** El defecto es de **forma**, no de contenido: A1 era el único nivel con algo de variedad (`{0:67, 1:36, 2:2}`) y **A2, B2, C1 y C2 estaban al 100 % en la posición 0**; el corpus de listening, en cambio, ya estaba equilibrado (~25 % por posición) y **no se toca**. **(B) La regla: reposicionamiento determinista, no rotación.** Dentro de cada grupo de checks con el **mismo número de opciones `k`**, ordenados por `id` ascendente, el check en la posición `j` del grupo lleva la correcta a **`j % k`**. Se implementa **moviendo la opción correcta** y no rotando el array: una rotación cíclica alteraría el orden lineal de los distractores al extraer la correcta (el elemento que la precede pasa a seguirla), mientras que el movimiento deja a los distractores **en su orden relativo original**, que es el mínimo cambio posible y respeta el orden en que se autoriaron. **(C) El instrumento, y por qué es un candado y no un script de un solo uso.** `backend/scripts/rebalance_mc_positions.py` (nuevo) es un instrumento de **contenido**, no una ruta de producto, con dos modos: `--write` aplica el reposicionamiento e `--check` **no escribe** y sale **1** si algún check no cumple la regla o si el reparto supera el **35 %**. La operación es **idempotente** (aplicarla a un contenido ya corregido no lo mueve), así que `--check` queda como tripwire re-ejecutable para la reautoría de V4.0.x. **(D) La cirugía: solo se sustituyen los textos de las opciones y el número.** La reescritura es **por líneas**, no un `json.dump`: reescribir el fichero entero arrastraría reformateo (listas cortas que el `dumps` expande, líneas en blanco) y haría el diff irrevisable. Cada ejecución comprueba **tres invariantes** antes de escribir: **forma canónica** (misma opción correcta y mismos distractores en el mismo orden relativo), **nº de líneas intacto** (delata cualquier blanco o pérdida de formato) y JSON válido. El resultado son **614 inserciones / 614 borrados** en los 6 niveles: puros intercambios, ni una línea de formato. **(E) El test deja de declarar el sesgo y pasa a fijar el invariante.** `test_mc_position_bias_of_curriculum_checks_is_declared` se reescribe como `test_mc_position_of_curriculum_checks_is_balanced`: mide el reparto **por grupo de `k`** —agregar los grupos escondería que el 4.º distractor (k=4) nunca fuese la correcta— y exige **≤ 35 % por posición** y **ninguna posición muerta**. Era exactamente lo que V3.70 pidió (un test que obligase a re-auditar el eje al corregirlo) y ahora muerde **en la dirección contraria**: una reautoría que vuelva a concentrar la correcta hace fallar la suite. **(F) Medición antes y después** (`python -m scripts.audit_dossier mc-bias`). Antes: `0:89,4 % · 1:10,1 % · 2:0,5 %`. Después: **`0:33,4 % · 1:33,2 % · 2:32,9 % · 3:0,5 %`**; por grupo, `k=3` queda `0:33,5 % · 1:33,2 % · 2:33,2 %` y `k=4` queda `0:30 % · 1:30 % · 2:20 % · 3:20 %`, con peor posición **33,5 %** bajo el límite. Los artefactos `docs/audit/generated/mc-position-bias.{md,json}` y `cefr-adequacy.{md,json}` se regeneran con los instrumentos que ya existían (solo lectura). **(G) `CURRICULUM_VERSION` 1.3.0 → 1.3.1.** Es **provenance pura**: la constante se **sella** en cada fila de evidencia y en cada snapshot de perfil, pero **nunca se compara** (verificado), así que subirla **no** invalida ni recalcula el estado de ningún alumno. Se sube porque el contrato declarado de la constante es «identifica **qué contenido** se evaluó» y el contenido servido cambia. **(H) Verificación.** Backend **2882 passed** (el **mismo recuento** que V3.75.0: el test se reformuló **en sitio**, no se añadió ni se retiró ninguno), `ruff` limpio sobre los ficheros tocados, `check_release_consistency` en los **6 orígenes** (`3.75.1`). **Sin cambios en frontend, launcher ni CI** (ninguno de los tres consume el orden de las opciones): la suite de frontend (721) y la del launcher (142) se declaran **no ejecutadas en esta release** porque no hay diff que las afecte, y `npm`/`pip-audit` no cambian porque no cambian las dependencias. **Honestidad.** (i) **El P2 de forma/longitud no se cierra y es deliberado:** la correcta sigue siendo la más larga en **39,1 %** de los checks y en el **50 %** del placement; reposicionar no toca longitudes, y ese es el hallazgo `#7` de `AA`, que sigue abierto. (ii) **`assessments.json` no se toca**: sus 22 ítems siguen con la correcta en la posición 0 en **63,6 %**; son otro instrumento, con su propia medición y su propio arreglo. (iii) Esto **elimina el atajo, no mejora los ítems**: los distractores son los mismos y la pista de la longitud sigue ahí para quien no entienda el enunciado. (iv) La asignación es estable **por revisión de contenido**, no por ítem: insertar o borrar un check desplaza la posición de los siguientes de su mismo `k`; `--check` es el candado que lo detecta. (v) Que solo **10 de 368** checks tengan 4 opciones es una irregularidad de autoría que este cambio no arregla. (vi) **Los 7 gates siguen en `pending`** y la identidad sellada en el kit sigue siendo la del pre-vuelo (`3.73.6` → `13cc30b`), que es historia y no se reescribe. Ver `release-notes-v3.75.1.md`.

- ✅ **V3.75.0 — La identidad la firma el servidor + cierre de VG-N5/VG-N6 (2026-09-18)** (**Versión estable `3.75.0`**, app `3.74.0 → 3.75.0`). **Release de PRODUCTO (minor): SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco y SIN tocar el currículum.** No añade ni retira un gate y no adelanta la validación física (G1–G7 siguen `pending`). Es la **Fase 2 del P0 de identidad** (`docs/audit/PLAN-P0-IDENTIDAD.md` §6 y §14): V3.74.0 cerró la **superficie** (dejar de exponerse por defecto); esta cierra la **identidad** (que el cliente no sea quien decide quién eres). **(A) El problema.** Hasta V3.74 el perfil activo viajaba en **cada** URL (`?user_id=…`) y el servidor aceptaba el que le pidieras, si existía; la cookie que recordaba la elección (`et_user_id`) la escribía **JavaScript**, así que se reescribía desde la consola del navegador y no era una credencial en ningún sentido. Con la frontera ya en loopback, quien alcanzara la API seguía **eligiendo a quién leer y escribir** con un parámetro. **(B) La solución: la firma el servidor.** `POST /api/session` comprueba que el perfil existe y emite `et_session`: `base64url(payload).base64url(hmac_sha256(secreto, payload))` con `payload = {iat, uid}`, **solo stdlib** (`itsdangerous` haría lo mismo con más superficie por treinta líneas) y comparación en **tiempo constante** (`hmac.compare_digest`; comparar firmas con `==` filtra por temporización). Atributos: **`HttpOnly`** (el cliente no puede leerla: ese es el punto), `SameSite=Lax`, `Path=/` y `Secure` cuando la petición va por HTTPS. `current_user` la verifica en cada petición: sin cookie, manipulada o caducada ⇒ **401 `SESSION_REQUIRED`**. **(C) El parámetro deja de significar nada.** `test_identity_source.py` manda a la vez **sesión de A** y `?user_id=B` y exige los datos de **A**; va marcado `identidad_cruda` porque el adaptador de `conftest.py` —que traduce el `?user_id=` de las **109** suites históricas a una sesión firmada, delegando en el camino real— haría inexpresable el caso «sin sesión + `?user_id=`». Lo que esas 109 suites dejan de probar se declara: lo cubren los tests nuevos. **(D) Autorización, no solo identidad.** `PATCH /api/users/{id}` y `PUT /api/settings` exigen sesión **y** que el id coincida con el de la sesión (**403** si no): antes, con sesión de A, se editaba el perfil de B. **(E) `et_user_id` se retira** con su test (`frontend/src/utils/cookie.ts`); el frontend **pregunta** (`GET /api/session`) en vez de creer, y elegir o crear perfil **abre sesión** — la decisión es una función pura con test propio (`utils/session.ts::planSession`), porque si la app pinta un perfil activo **sin** sesión todo se ve normal y **cada** petición responde 401. Los **17** módulos de `api/` dejan de añadir `?user_id=…` (parámetro renombrado a `_userId`: `noUnusedParameters` está activo y retirarlo del todo es la Fase 2b, declarada y **no** hecha). **(F) El secreto de firma no viaja en los backups.** `session.secret` entra en `_NON_PORTABLE_TOP_NAMES`: un ZIP sin cifrar con la clave dentro permite **forjar sesiones**; restaurar **no** borra el secreto del equipo receptor, así que las sesiones abiertas siguen valiendo. **(G) El launcher deja de imprimir el token.** Informa de «sesión abierta: sí/no» y **enmascara** el valor de la cookie: Firefox las guarda en claro y verla en pantalla sería poder usarla (el acoplamiento que el plan declaró en §7.1). **(H) `VG-N5`: cadena de suministro y dependencias.** Actions fijadas por **SHA de commit** con la etiqueta en comentario (`@v4`/`@v5` son referencias **móviles**), `permissions: contents: read`, job **bloqueante** `deps-audit` (`pip-audit -r requirements.txt` + `npm audit --omit=dev --audit-level=high`) y **Dependabot** para `pip`, `npm` y `github-actions` (el tercero mantiene frescos los pines). **El escaneo destapó deuda real:** 10 avisos conocidos en `starlette 0.50.0` (arrastrado por `fastapi==0.128.0`), con parches solo en `starlette>=1.0.1` ⇒ `fastapi` sube a **0.141.1** (`starlette` **1.6.0**) y la suite completa pasa idéntica (2875 passed en el árbol intermedio, antes de los tests nuevos). **(I) `VG-N6`: la superficie sin credencial, declarada y acotada.** `/api/system/status`, `/api/network` y `/api/models` se **aceptan por escrito** (`docs/ARQUITECTURA.md` §«Superficie sin sesión»): exponen reconocimiento barato —modelos instalados, IP/hostname de LAN, trabajos de generación en curso, rechazos por rate limit de 60 s— que **no** permite leer ni escribir datos de ningún alumno, y quien puede alcanzarlas ya está dentro para lo demás (la frontera real es la red). Candado en **las dos direcciones** (`test_public_surface.py`): lo declarado sin sesión **sigue** respondiendo (si no, se rompe el launcher o la puerta de perfil, que funcionan antes de que exista ningún perfil) y lo declarado con sesión **no** sale sin ella; borrar la sección del documento **hace fallar** el test. **(J) Verificación.** Backend **2882 passed**, frontend vitest **721** (87 ficheros), launcher **142**, `ruff`/`tsc` limpios, i18n `--strict` **0/0/0**, `validation_gate.py auto` **10/10**, `check_release_consistency` en los **6 orígenes** (`3.75.0`), `npm audit`/`pip-audit` sin vulnerabilidades conocidas y **CI de 12 jobs** (el nuevo, `deps-audit`). Los recuentos suben exactamente por los tests nuevos (backend 2840 + **42**, frontend 719 + **2** netos, launcher 139 + **3**). Se comprobó la **mordida** de cinco candados nuevos por sabotaje controlado: etiqueta móvil de Action, job de auditoría informativo, declaración de superficie borrada, `HttpOnly` quitado y adaptador de tests traduciendo las peticiones crudas — los cinco hacen fallar su test. **Honestidad.** (i) **Esto no es autenticación y la Fase 3 sigue sin decidir:** `POST /api/session` acepta cualquier `user_id` **existente** sin credencial y `GET/POST /api/users` siguen sin ella, así que quien alcance la API (en modo LAN) puede **abrir sesión para cualquier perfil**; lo que ya no puede es **forjar** una identidad sin el secreto del equipo ni **elegirla por petición**. Cerrar el acceso exige credencial y contradice «sin cuentas, sin contraseñas» de `docs/PREMISAS.md`: es decisión de producto. (ii) El token prueba que lo **emitió el servidor**, no que quien lo pide tenga derecho al perfil. (iii) El salto a `starlette` **1.6.0** es una versión mayor dentro de una release de seguridad: se hizo porque dejarlo era publicar 10 avisos conocidos, y la evidencia es la suite completa en verde, no la lectura de un changelog ajeno. (iv) La `Secure` de la cookie depende del esquema: en el uso real (HTTPS) va puesta; servido por HTTP plano, no. (v) El candado de `VG-N6` fija una **lista**, no demuestra que sea completa. (vi) **Los 7 gates siguen en `pending`** y la identidad sellada en el kit sigue siendo la del pre-vuelo (`3.73.6` → `13cc30b`), que es historia y no se reescribe. Ver `release-notes-v3.75.0.md`.

- ✅ **V3.74.0 — Frontera de red: loopback por defecto y LAN opt-in (2026-09-18)** (**Versión estable `3.74.0`**, app `3.73.7 → 3.74.0`). **Release de PRODUCTO (minor): SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco y SIN tocar el currículum.** No añade ni retira un gate y no adelanta la validación física (G1–G7 siguen `pending`). Cierra la **mitigación `R1`** del P0 de identidad: hasta V3.73.6 el producto se enlazaba **siempre** a `0.0.0.0` y la regex de CORS aceptaba **cualquier** IP privada, así que exponer los datos del alumno a la WiFi era el **comportamiento por defecto** y nadie lo había pedido. **(A) El problema, con su alcance medido.** La frontera de red no se elegía: se heredaba. Con `bind 0.0.0.0` + CORS de red privada, cualquier equipo de la misma red alcanzaba la API, y como el P0 de identidad sigue abierto (el `user_id` lo elige el cliente y `POST/PATCH/DELETE /api/users` no exigen credencial), ese acceso era **lectura y escritura** de los datos del alumno. **(B) Lo que cambia: la frontera pasa a ser una decisión.** Uvicorn se enlaza a `127.0.0.1` salvo que el modo esté **declarado** (`ENGLISH_TUTOR_LAN=1`, o el botón «Activar red local» del panel de acceso del launcher, que lo declara y **reinicia el servidor** para aplicarlo); `lan_mode()` se lee **fail-closed** (ausente o no afirmativo ⇒ cerrado) y el launcher lo propaga en el entorno del backend, de modo que una **sola** decisión gobierna el `--host` de uvicorn (`backend_host`) y la política de orígenes — no pueden discrepar. El arranque lo deja en el log (`Modo de red: LAN | solo loopback`) para que «no me entra desde el móvil» tenga causa escrita en lugar de hipótesis. **(C) La trampa que se evitó (corrección al plan).** El plan inicial decía que bastaba con vaciar `ALLOWED_ORIGIN_REGEX`: era **falso y peligroso**, porque `origin_allowed` comprobaba con `_ORIGIN_RE.match(...)` y un patrón **vacío** casa con **cualquier** cadena — «desactivar» la validación habría **abierto** CORS en lugar de cerrarlo. Ahora hay dos patrones separados (`LOCAL_ORIGIN_REGEX`, siempre válido; `LAN_ORIGIN_REGEX`, solo en modo LAN), la comprobación es `fullmatch` y el modo se consulta **por petición** en lugar de cachearse al importar. **(D) Sin enlaces muertos.** `/api/network` informa del modo (`lan_mode`, `bind`) y **no** anuncia la URL de LAN cuando no responde; el panel del launcher muestra «desactivada (solo este equipo)» en vez de un enlace que no funciona; y `ConnectDeviceCard` explica el paso que falta (`connect.lanOff`/`connect.lanOffHow`) en lugar de pintar un QR inútil. **(E) Candados.** `test_las_dos_mitades_de_la_politica_de_origen_coinciden` compara el patrón de `CORSMiddleware` con `security.origin_allowed` sobre los mismos orígenes y falla si discrepan (es el acoplamiento que un cambio futuro puede romper en silencio, porque el middleware compila su regex **una vez** al importar); los tests de `lan_mode` fijan el fail-closed y los valores afirmativos; y hay candado de **deriva documental**: si la frontera desaparece de `README.md`, `PREMISAS.md` o `ARQUITECTURA.md`, la suite falla. **(F) Fase 1b: el botón, con la lógica fuera de la GUI.** «Activar/Desactivar red local» declara el modo (`core.set_lan_mode`), refresca la fila y **reutiliza el «Reiniciar servidor» que ya existía**; con la app parada solo deja el modo declarado y lo dice. La decisión vive en `core.py` y no en `tkinter` porque la GUI no se puede probar sin pantalla: así la decisión que cambia la frontera de red **tiene test** (ida y vuelta declarar → `backend_host` → entorno del proceso). **(G) Verificación.** Backend **2840 passed**, frontend vitest **719** (86 ficheros), launcher **139**, `ruff`/`tsc` limpios, i18n `--strict` **0/0/0**, `validation_gate.py auto` **10/10** y `check_release_consistency` en los **6 orígenes** (`3.74.0`); los recuentos suben exactamente por los tests nuevos (backend 2817 + **23**, frontend 717 + **2**, launcher 113 + **26**). **Honestidad.** (i) **No es autenticación y el P0 sigue abierto:** en modo LAN, `/api/users` sigue enumerando y creando perfiles **sin credencial** y el `user_id` lo sigue eligiendo el cliente; el `et_user_id` no desaparece hasta la **Fase 2** (alcance y plan en `docs/audit/PLAN-P0-IDENTIDAD.md`). (ii) El modo **no se persiste** a propósito: es una variable de entorno, para que exponerse exija un acto explícito en cada arranque. (iii) El botón **reinicia el servidor** (la app se recarga); no es un ajuste en caliente. (iv) El candado de coherencia cubre una **lista** de orígenes, no demuestra que sea exhaustiva. (v) **Los 7 gates siguen en `pending`** y la identidad sellada en el kit sigue siendo la del pre-vuelo (`3.73.6` → `13cc30b`), que es historia y no se reescribe. Ver `release-notes-v3.74.0.md`.

- ✅ **V3.73.7 — Endurecimiento derivado de la verificación de seguridad (2026-09-18)** (**Versión estable `3.73.7`**, app `3.73.6 → 3.73.7`). **Release de PARCHE: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco y SIN tocar el currículum**, pero **SÍ con cambios de PRODUCTO** (backend, frontend y un middleware nuevo), a diferencia de V3.73.2–V3.73.6, que eran documentales. No añade ni retira un gate y no adelanta la validación física. **(A) El origen: el triage interno de la auditoría de seguridad.** `docs/audit/VERIFICACION-SEGURIDAD-V373.md` verificó hallazgo a hallazgo la sección de seguridad del informe externo sobre `v3.73.6`, separó confirmados, matizados y refutados, y de ahí salió un lote acotado de correcciones; **el P0 de identidad no se toca** (§Honestidad). **(B) El bug que sostiene la prioridad 1: un límite que no se aplicaba.** `_PATH_LIMITS` declaraba `/api/voz/transcribe`, una **ruta que no existe** (el router monta `/api/transcribe`), así que Whisper usó el cupo general (**1200/min**) en vez del reforzado (**180/min**) que el propio código declaraba; caían también en el general `/api/tts`, `/api/translate` y `/api/voices/download`, las tres sin credencial. Corregido, con cupo propio para las cuatro. **(C) La causa raíz, no el síntoma.** Las claves de `_PATH_LIMITS` eran cadenas sueltas **sin candado**: `test_path_limits_match_real_routes` recorre ahora la tabla de rutas **real** de la app (descendiendo por los `_IncludedRouter` con que FastAPI difiere `include_router`, con suelo mínimo de rutas para no pasar «por vacío») y falla si alguna clave deja de corresponder a una ruta montada; `test_costly_routes_have_own_rate_limit` impide que una ruta cara pierda su cupo. Comprobado que el inventario ve **163 rutas** y que la clave vieja **no casa con ninguna** —es decir, el test habría cazado el bug original. **(D) El backup dejaba salir la clave privada TLS.** `_write_zip` recorría todo `DATA_DIR` e incluía `data/certs/key.pem`: la clave del certificado con el que el producto sirve HTTPS en la LAN viajaba **en claro** en cada copia, multiplicada por las **7** automáticas. Ahora `certs/` queda fuera del ZIP y **se conserva** al restaurar, gobernado por **una sola** constante (`_NON_PORTABLE_TOP_NAMES`) para que no se pueda excluir del backup y olvidar protegerlo en la restauración; restaurar un backup viejo tampoco revive el certificado. **(E) Restaurar no tenía cota de expansión.** El tope de **512 MB** del router es del ZIP **comprimido**: sin cota del expandido, un ZIP de pocos MB con ratio alto llena el disco de `%TEMP%`. `_validate_archive` añade cota de tamaño descomprimido (**4 GiB**), cota de entradas (**50 000**) y rechazo **explícito** de rutas inseguras (`..`, absolutas, unidad `C:`), sin depender de que `extractall` de Python 3.13 sanea los nombres. **(F) Cuatro endurecimientos menores.** `UserCreate.name` con `max_length=80` (solo `PATCH` lo tenía, y `POST /api/users` **no exige credencial**); cookie de perfil con `Secure` cuando la página va por HTTPS; middleware `SecurityHeadersMiddleware` con `nosniff`, `X-Frame-Options: DENY`, `Referrer-Policy`, `Permissions-Policy` y **CSP acotada**; y el PIN de administración pasa de `localStorage` (en claro, para siempre, compartido por todos los perfiles) a `sessionStorage`, con borrado de la copia heredada — es el único cambio **visible para el usuario**: el PIN se teclea una vez por sesión de navegador. **(G) Verificación.** Backend **2817 passed** (0 skipped, árbol de trabajo con `dist` y modelos), frontend vitest **717** (85 ficheros), launcher **113**, `ruff`/`tsc` limpios, i18n `--strict` 0/0/0, `validation_gate.py auto` **10/10** y `check_release_consistency` en los **6 orígenes**; los recuentos suben exactamente por los tests nuevos (backend 2798 + **19**, frontend 712 + **5**). Se comprobó que los tests **muerden**: con el fuente del backup en un stash temporal fallan **7 de los 8** nuevos, y el octavo (conservación del certificado al restaurar) se verificó aislando el lado de la restauración. **Honestidad.** (i) **El P0 de identidad sigue abierto**: el `user_id` lo sigue eligiendo el cliente y los endpoints de perfil siguen sin credencial; esta release cierra el lote barato y acotado, **no** el riesgo principal. (ii) La CSP es **parcial** a propósito (`frame-ancestors`, `object-src`, `base-uri`, `form-action`): el artefacto trae un script inline de tema, así que cerrar `script-src` exige un hash y su propio candado. (iii) **No** se envía HSTS: con certificado autofirmado dejaría el origen inutilizable. (iv) **No** es un pentest: son correcciones con el test que falla sin el cambio. (v) **Los 7 gates siguen en `pending`** y la identidad sellada en el kit sigue siendo la del pre-vuelo (`3.73.6` · `13cc30b`), que es historia y no se reescribe; si la campaña física se ejecuta después de este commit, sus registros sellarán el SHA nuevo. Ver `release-notes-v3.73.7.md`.

- ✅ **V3.73.6 — Reproducibilidad de las cifras declaradas de la auditoría (2026-09-17)** (**Versión estable `3.73.6`**, app `3.73.5 → 3.73.6`). **Release de PARCHE: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN cambios de PRODUCTO** (backend de producto, frontend de producto y launcher intactos, y el arnés `scripts/validation_gate.py`, sus tests y el contrato de los 7 gates **sin tocar**: el diff es **documentación**). No añade ni retira un gate y no adelanta la validación física. **(A) El origen: un pre-vuelo del auditor.** Antes de lanzar la auditoría se clonó el repositorio **desde GitHub** y se ejecutaron, uno por uno, los comandos que el punto de entrada manda ejecutar, para que el auditor no gastara una ronda en un falso positivo. **(B) El hallazgo: una cifra no reproducible.** El documento declaraba `pytest tests/ -q # 2796 passed + 2 skipped (2798 casos) en el worktree limpio` como el recuento del **checkout limpio** —que es la autoridad— y un clon recién hecho da **2795 passed · 3 skipped**. Medido en tres entornos: clon limpio **2795 + 3**, ese clon tras `npm run build` **2796 + 2** y árbol con `dist`, modelos Whisper y BD local **2798 + 0**. El **invariante es 2798 casos** y nunca estuvo mal; lo que fallaba era el **reparto** y su **causa**. **(C) La causa estaba mal atribuida.** El documento decía «condicionales del banco de escenario»; los saltos son de **artefactos no versionados**: `frontend/dist` (1, en `test_serve_frontend_v372.py:173`) y el **modelo Whisper opt-in** (2, en `test_stt_asr_integration.py:44,55`). La cifra de 2796 se había medido en un worktree que **sí** tenía `dist` construido —de ahí que allí no apareciera el salto del artefacto— y se publicó como la del checkout limpio. **(D) El arreglo.** El punto de entrada declara ahora el **invariante** (2798 casos) y el **reparto por entorno** con los motivos exactos medidos, más el aviso explícito de que **2795 + 3 en un clon sin compilar no es un hallazgo**; la corrección se marca como **errata (V3.73.6)** dentro del documento y con una línea de errata en las notas de V3.73.4 y V3.73.5, **sin reescribir el histórico** (las releases anteriores quedan como registro de lo que declararon); y los **11 nombres de job** del CI, que iban partidos entre líneas y hacían fallar un `grep` literal, pasan a ir **uno por línea**. **(E) Verificación (pre-vuelo sobre clon nuevo desde GitHub).** `main` == tag; `git diff --stat v3.73.6..main -- backend frontend launcher scripts` **vacío**; run del commit del tag **`success` con 11/11**; `check_release_consistency` OK; `status --strict` **exit 1** y `auto` sin `dist` con la comprobación 9 en `skip` (los dos, como se declara); recuentos declarados comprobados uno a uno (43/19/23/19 = **104**; launcher **113** y **13 + 7 = 20**); en el árbol de trabajo, backend **2798 passed**, frontend vitest **712 passed** (85 ficheros), launcher **113**, `ruff`/`tsc` limpios, i18n `--strict` **0/0/0**, `auto --require-dist` **10/10** y `check_release_consistency` en los **6 orígenes** (`3.73.6`). **Honestidad.** (i) Esta release **no** mueve ningún gate y **no** sustituye al auditor: verifica que sus comandos no dan falsos positivos y que las cifras declaradas son reproducibles, **no** juzga el producto; (ii) el error real no fue escribir un número, sino **declarar como reproducido lo que solo se había medido en otro entorno**, y la cifra se había propagado desde V3.73.4 (y repetido sin medir en la nota de V3.73.5 de `docs/RELEVO.md`), así que se corrige con errata y no borrando el rastro; (iii) **los 7 gates siguen en `pending`** y `status --strict` sigue saliendo **1**, que es lo correcto: el siguiente hito sigue siendo ejecutar físicamente G1–G7. Ver `release-notes-v3.73.6.md`.

 - ✅ **V3.73.5 — Cierre documental del ancla de la auditoría (2026-09-17)** (**Versión estable `3.73.5`**, app `3.73.4 → 3.73.5`). **Release de PARCHE: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN cambios de PRODUCTO** (backend de producto, frontend de producto y launcher intactos: el diff es **documentación**). No añade ni retira un solo gate, no toca el arnés `scripts/validation_gate.py` ni sus tests: lo único que cambia es **dónde se ancla** el punto de entrada de la auditoría externa y **cómo se declara** el estado de publicación. **(A) El problema, con su causa raíz.** Hasta V3.73.4 el punto de entrada (`agentes/auditoria-total-externa-v373.md`) se publicaba **fuera del tag que él mismo declaraba**: el re-anclaje era un commit **posterior** al tag, y el documento fijaba a mano el SHA del commit de release, el objeto del tag anotado y el id del run de CI. Los tres datos son **imposibles de tener cuando se escribe el commit** —un commit no contiene su propio SHA ni el id de la run que dispara su push—, así que el documento nacía desfasado y cada cierre generaba un re-anclaje nuevo que volvía a quedar fuera del tag siguiente. No era un descuido de redacción: era **estructural**, y estaba admitido como «se re-ancla en cada cierre» en la propia nota de historial. **(B) Cierre: el ancla es el tag y el estado se verifica por comando.** El documento pasa a declarar la release auditada **por su tag** y a entregar los comandos que resuelven el commit y el objeto del tag (`git rev-parse`), en lugar de fijar valores que solo existen después de publicar; el run de CI deja de ser un dato congelado y pasa a ser una **comprobación reproducible** (`gh run list --commit <sha>`) que el auditor ejecuta en el momento de auditar. El **invariante del código** se enuncia entre el tag y `main` —que es exactamente lo que el auditor puede comprobar— y la nota de historial describe la causa raíz en vez de repetir la admisión. Consecuencia buscada: el commit de release es **final** (no hay commit documental posterior al tag), de modo que el tag **contiene** el punto de entrada que declara y `main` deja de ir por delante en documentación. **(C) Sin efectos sobre el instrumento.** `scripts/validation_gate.py`, sus tests, los **7 gates** y `docs/audit/validation-evidence.json` quedan idénticos a V3.73.4; el informe determinista `docs/audit/generated/release-validation.{md,json}` solo cambia en la versión que declara. **(D) Verificación.** `check_release_consistency` en los **6 orígenes** (`3.73.5`), tests de deriva documental, `validation_gate.py auto --require-dist` **10/10** y regeneración idéntica del informe; detalles en `release-notes-v3.73.5.md`. **Honestidad.** (i) Esta release **no** mueve ningún gate y **no** adelanta la validación física: G1–G7 siguen siendo la única cosa que falta para V4.0. (ii) El desfase de V3.73.4 se **documenta como error de diseño cerrado**, no se reescribe: el tag `v3.73.4` y su run se conservan intactos y quedan como base de comparación.

- ✅ **V3.73.4 — Recorder trazable (2026-09-17)** (**Versión estable `3.73.4`**, app `3.73.3 → 3.73.4`). **Release de PARCHE: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN cambios de PRODUCTO** (backend de producto, frontend de producto y launcher intactos: el diff es el **arnés de validación**, sus **tests** y **documentación**). Cierra el eslabón que faltaba en la cadena de certificación: la evidencia de un gate **no decía contra qué commit se había probado**, de modo que siete gates verdes en siete commits distintos se podían presentar como «los siete gates». **(A) La evidencia sella el commit:** `record` escribe `head_sha` (de `git rev-parse HEAD`) junto a estado, notas, fecha UTC y `VERSION`, y acepta `--ci-run` (id numérico o URL de la run; se guarda el id) — la cadena `commit → run → artefactos → gate records` queda dentro de `docs/audit/validation-evidence.json`; un **`pass` sin commit se rechaza** (sin git en el árbol no hay registro), mientras `fail`/`skip`/`pending` sí se registran sin SHA porque declaran un no-cierre. **(B) La puerta fuerte de V4.0:** `status` imprime el commit por gate y marca la evidencia de otro árbol, `--strict` sigue significando «7/7» (el CI no cambia) y **`--strict --same-tree`** exige además que los siete `head_sha` sean el commit actual. **(C) `auto` vigila el formato:** la comprobación 10 (`evidencia-integra`) falla si un `pass` no trae `head_sha` o si el `head_sha` (40 hex) o el `ci_run` (dígitos) son inválidos. **(D) Una sola cifra de gates humanos:** `Gate.human` se declara **gate a gate** (sin valor por defecto) y vale `True` en los **7** —el instrumento no ejecuta ningún flujo de la app—, y la expresión «7 gates (5 de ellos acción humana)» de notas históricas de V3.73.0 (que describía los cinco bloques físicos de V3.72) queda explicada en el runbook, sin dos cifras en circulación. **(E) Documentación:** el kit y el runbook explican qué sella la evidencia, la regla del `pass` con commit, la puerta `--same-tree` y la tabla de identidad (`VERSION`/`HEAD`/run) del pre-vuelo, y los 7 comandos `record` de la planilla llevan `--ci-run <run>`. **Tests y gates:** backend **2796 passed · 2 skipped** (2798 casos) en el **`git worktree` limpio del pre-vuelo** sobre el tag —los 2 saltos son condicionales del banco de escenario, que en el worktree no tiene la BD local (no versionada)— y **2798 passed** (0 skipped) en el árbol de trabajo (misma cifra de casos, 2798); `test_validation_gate_v373.py` 29 → **43** (+14 casos: SHA sellado, `pass` sin git rechazado, normalización y validación de `--ci-run`, `--same-tree` con evidencia propia/ajena/sin git, y formato de `head_sha`/`ci_run` en la evidencia), `ruff` limpio; frontend sin cambios de producto (`tsc` limpio, vitest **712 passed**, `npm run build` OK); `check_release_consistency` en los **6 orígenes** (`3.73.4`); `auto --require-dist` **10/10** (en el worktree, con el informe regenerado **idéntico** al commiteado); `status --strict` **exit 1** (correcto). **Los 7 gates siguen en `pending`** y el código sigue congelado: el siguiente hito es ejecutar G1–G7. Ver `release-notes-v3.73.4.md`. **Errata (V3.73.6):** la cifra de test declarada en esta entrada («backend **2796 passed · 2 skipped** (2798 casos) en el `git worktree` limpio… los 2 saltos son condicionales del banco de escenario») se corrige: el **invariante son 2798 casos** y el reparto depende de **artefactos no versionados** —clon limpio **2795 + 3**, tras `npm run build` **2796 + 2**, árbol completo **2798 + 0**—, porque los saltos son del `frontend/dist` (1) y del **modelo Whisper opt-in** (2), no del banco. Ver `release-notes-v3.73.6.md`.

- ✅ **V3.73.3 — Kit de validación de los 7 gates (2026-09-17)** (**Versión estable `3.73.3`**, app `3.73.2 → 3.73.3`). **Release de PARCHE: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN cambios de PRODUCTO** (backend de producto, frontend de producto y launcher intactos: el diff es el **arnés de validación**, **tests** y **documentación**). Prepara lo único que falta para V4.0 —**ejecutar físicamente los 7 gates**— y corrige la deriva que habría hecho probar el artefacto equivocado. **(A) Kit de campo:** nuevo `docs/audit/KIT-VALIDACION-GATES.md` con pre-vuelo (build, identidad del árbol que se sella en cada `record`, manifiesto offline), reglas del instrumento, **hoja por gate** (G1–G7: protocolo enlazado, precondiciones, pasos, qué cuenta como FALLO, evidencia y el `record` copiable), agrupación **por sesión** y cierre; **referencia** los protocolos en lugar de duplicarlos (la definición sigue en `docs/audit/VALIDATION-RELEASE-V373.md`). **(B) Deriva de protocolo corregida — habría invalidado G1 y G4:** el §5 de `RA-RUNTIME-OFFLINE.md` y `G-DEVICES.md` seguían mandando al **dev server de Vite** (`:5173`), que desde V3.72 **no es el runtime de producto** (un solo origen HTTPS `:8000` con el backend sirviendo `dist`), de modo que seguirlos al pie de la letra habría probado **otro artefacto**; `RB-INSTALACION.md` declara Node/npm como requisito de **compilación**, no de ejecución. **(C) Guard anti-deriva:** `check_device_matrix` se generaliza a `check_gate_protocol_origins` (id `gate-origins`), que recorre los **tres** protocolos funcionales (`DEVICE_MATRIX.md`, `RA-RUNTIME-OFFLINE.md`, `G-DEVICES.md`) y **falla si alguno contiene `:5173` o no contiene `:8000`** — el chequeo que vigilaba un solo documento ya no deja pasar la deriva por la puerta de al lado (**5 tests nuevos**; `test_validation_gate_v373.py` 24 → **29**). **(D) Punto de entrada de la auditoría externa, re-anclado en el cierre:** `agentes/auditoria-total-externa-v373.md` afirmaba un invariante de código (`git diff --stat v3.73.0..main -- backend frontend launcher scripts` **vacío**) que V3.73.1/V3.73.2 ya habían roto —un **P0 falso** en el primer comando del auditor, según sus propias reglas— con cifras y versión caducas; se re-ancla al tag de esta release con el invariante **verificado** y el kit en el orden de lectura, una vez comprobado el CI de publicación. **Tests y gates:** backend **2781 passed · 3 skipped** (2784 casos) verificado en un **`git worktree`** sobre el commit de release, más `ruff` limpio; frontend sin cambios de producto (`tsc` limpio, vitest **712 passed**, `npm run build` OK); launcher **113**; `check_release_consistency` en los **6 orígenes** (`3.73.3`); i18n `--strict` **0/0/0**; `validation_gate.py auto --require-dist` **10/10**; `status --strict` **exit 1** (correcto). **Los 7 gates siguen en `pending`** y el código sigue congelado: el siguiente hito es ejecutar G1–G7. Ver `release-notes-v3.73.3.md`.

- ✅ **V3.73.2 — Corrección de la CI de V3.73.1 (2026-09-17)** (**Versión estable `3.73.2`**, app `3.73.1 → 3.73.2`). **Release de PARCHE correctiva: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN cambios de PRODUCTO** (backend de producto, frontend de producto y launcher intactos: solo **tests e instrumento de auditoría** del backend más documentación). V3.73.1 se publicó (`59e731d`, tag `v3.73.1`) con **10 de 11 jobs verdes y `Backend (ruff + pytest)` en rojo** (`1 failed, 2775 passed`): `test_ped_coverage_v370::test_reading_has_no_dedicated_scorer` afirmaba que existe `frontend/src/features/reading/` —la parte «UI sí» de la asimetría de `reading`— y el borrado de `ReadingPractice` (§6 de V3.73.1) dejó ese directorio **vacío**, de modo que en el disco del autor seguía existiendo (suite local **2779 passed**) y en un checkout limpio no (CI **1 failed**). **(A) Reapunte, no debilitamiento:** desde la Opción A la UI de `reading` es el **chat con destreza** `/chat/lectura`; `backend/scripts/audit_dossier.py` declara `reading.ui = chat:lectura` y resuelve la existencia con la tabla explícita `UI_ARTIFACT_PATHS` (tipo + ruta) en lugar de asumir siempre un directorio de `frontend/src/features`, y los otros **8** artefactos conservan **exactamente** el `is_dir()` previo; el test comprueba el **cableado real** (`frontend/src/router/chat.ts` declara `reading: "lectura"`) y que el dossier siga viendo esa UI (`ui_exists true`) manteniendo `scorer_exists false` y `corpus 0`. **(B) Dossier:** `skill-coverage.{json,md}` regenerados (solo cambia la **procedencia** de la UI de `reading`; **sin** gap espurio de «sin feature de UI») y `AB-PED-COBERTURA.md` actualiza la evidencia del hallazgo 4. **(C) Lección de proceso, declarada:** la verificación local era **estructuralmente incapaz** de ver este fallo por el directorio vacío, así que la verificación de V3.73.2 se hace sobre un **checkout limpio de verdad** (`git worktree`), no sobre el disco del autor. **Tests y gates:** frontend `tsc` limpio, vitest **712 passed**, `audit:contrast` **0 fallos bloqueantes**, `npm run build` OK; backend **2776 passed · 3 skipped** (2779 casos) verificado en checkout limpio y dossier **estable** (0 diff); `ruff` limpio; `check_release_consistency` en los **6 orígenes** (`3.73.2`); i18n `--strict` **0/0/0**; `validation_gate.py auto --require-dist` **10/10**; `status --strict` **exit 1** (correcto). **Los 7 gates siguen en `pending`** y el código sigue congelado: el siguiente hito es ejecutar G1–G7. Ver `release-notes-v3.73.2.md`.

- ✅ **V3.73.1 — Cierre GUI pre-V4.0 (2026-09-17)** (**Versión estable `3.73.1`**, app `3.73.0 → 3.73.1`). **Release de PARCHE: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN tocar backend ni launcher: todo el cambio es frontend.** Cierra los **seis hallazgos GUI/UX (P2/P3)** que el dictamen externo de cierre de V3.73.0 dejó abiertos antes de V4.0, sin abrir arquitectura. **(A) APRENDER se ordena (GUI-01, GUI-02, GUI-04 — Opción A):** las 4 tarjetas pasan a `xl:grid-cols-4` (antes la cuarta quedaba aislada en un grid de 3 columnas), el `aria-labelledby` del grid apunta por fin a un nodo real (`SectionHeading` acepta `id`) y **Reading/Writing dejan de ser superficies huérfanas**: nuevo `router/chat.ts` (`ChatSkill`, `chatSkillPath` → `/chat/lectura` y `/chat/escritura`, `chatSkillFromPath`), `routeMap` acepta ese segundo segmento (cualquier otro degrada a `home`), `App` selecciona la sección que trae la URL —así las **recomendaciones** del motor también abren el chat con su contexto—, `PracticeView` incluye `reading` en la rama de chat con su `kicker` y `Workspace` deja de resaltar Speaking cuando la URL trae destreza; `LearnHub` gana el bloque secundario **«Practica con el tutor»** (Reading y Writing) con tap target y `focus-visible`. Antes, elegir Reading abría **Speaking** (el efecto de `/chat` forzaba la sección). **(B) Código muerto e i18n:** `SectionNav.tsx` (no se importaba) y `ReadingPractice.tsx` (inalcanzable) **borrados** con sus claves huérfanas (`nav.skills`, `group.primary`, `group.support`, familia `reading.*`); `check_i18n_coverage.py --strict` sigue en **0/0/0** (1477 cadenas); se conservan `ReadingIcon`/`WritingIcon` y `WritingPanel`/`WritingJourney`; el CSS legacy `.reading-practice-*`/`.reading-card-*`/`.reading-academy-*` **no se toca** (limpieza → V4.0.x, declarado). **(C) Navegación móvil (GUI-03/GUI-08):** se mantienen los **5 destinos** y los touch targets (`min-h-14`), pero el núcleo (Inicio/Formación/Aprender) deja de competir con las utilidades (Diccionario/Traductor): **divisor real** + peso visual menor para los auxiliares. **(D) `prefers-reduced-motion` (GUI-05):** `<MotionConfig reducedMotion="user">` envuelve la app, de modo que **todas** las animaciones `motion/react` respetan el sistema (antes solo `legacy.css`), con guarda Playwright. **(E) Contraste medido y arreglado lo arreglable (GUI-06):** nuevo `frontend/scripts/contrast_audit.mjs` (`npm run audit:contrast`, **en CI**) que lee los tokens reales y mide **184 pares + 2 guardas** → `docs/audit/generated/contrast-report.{json,md}`; **bloqueante** (tipografía base + texto de acento) **0 fallos**, **reportado** (relleno + tinta, borde) **17 de 42 con el máximo alcanzable de cada acento**. Se corrige `--color-text-faint` claro (`#6f7c90 → #616e84`, mínimo 4.55:1) y sobre todo **el texto de acento deja de ser un hex fijo**: `--color-accent-soft` **se deriva del acento elegido** con `color-mix(in srgb, var(--color-accent) var(--accent-soft-share), var(--accent-soft-target))` (64/67/65/56/56/64/52 % según acento), porque **no seguía al acento** (con turquesa salía índigo) y con ámbar en claro quedaba en **2.02:1**; las ~20 reglas que usaban el acento **sólido como texto** pasan a ese token. **Guardas:** prohibido `color: var(--color-accent)` como texto, obligada la derivación, y `accentContrast.spec.ts` verifica **en el navegador real** los 14 pares acento×tema (≥4.5:1 sobre las tres superficies), que es lo único que detecta un `color-mix()` inválido en runtime. **Honestidad:** el **relleno de acento con su tinta no puede llegar a AA** con la rampa actual (máximo alcanzable 3.41–4.19 con la mejor tinta) ni el acento como borde en claro (turquesa 2.49, esmeralda 2.54, ámbar 2.15 frente a 3:1): exige **re-rampar los 7 acentos** y se decide en **V4.0.x** con la tabla delante; **no se declara WCAG 2.2 AA certificado**. **(F) Teclado y zoom:** `keyboard.spec.ts` (skip link → `#main-content`, tarjetas del hub —incluidas las del tutor— solo con teclado) y `reducedMotionAndZoom.spec.ts` (sin desborde horizontal ≤1 px a 200 % de zoom). **(G) Tests y gates:** `tsc` limpio, frontend **712 passed** (85 ficheros, 699 → **+13**), Playwright **38 passed · 28 skipped · 0 failed** (13 specs × 3 breakpoints, a **4 workers**: la corrida a 16 workers dio 13 fallos **solo en desktop** por contención del dev server en frío y los 13 pasan al repetirlos), `npm run build` OK, `check_release_consistency` en los **6 orígenes** (`3.73.1`), i18n `--strict` **0/0/0**, `validation_gate.py auto --require-dist` **10/10** y `status --strict` **exit 1** (correcto). Backend y launcher **no se tocan**. **Los 7 gates siguen en `pending`**: este parche **no** hace la validación física y **congela el código**; el siguiente hito es ejecutar G1–G7. Ver `release-notes-v3.73.1.md`. **Corrección (V3.73.2):** esta release salió con la **CI en rojo** (job `Backend (ruff + pytest)`, `1 failed`) porque el borrado de `ReadingPractice` dejó **vacío** `frontend/src/features/reading/` y **git no versiona directorios vacíos**, así que el directorio no existe en un checkout limpio; el invariante se reapuntó en V3.73.2 sin tocar producto. Ver `release-notes-v3.73.2.md`.

- ✅ **V3.73.0 — Validation release (2026-09-17)** (**Versión estable `3.73.0`**, app `3.72.0 → 3.73.0`). **Release de VALIDACIÓN: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN capacidad pedagógica nueva.** Cierra el endurecimiento mínimo que la auditoría de V3.72 dejó como P2/P3 y sobre todo construye el **instrumento de certificación** que convierte los 7 gates de validación física (5 de ellos acción humana) en **evidencia registrada y exigible**. **(A) Fail-closed del runtime de producto (P2):** `mount_frontend` acepta `require_ui`; en producto (el launcher lo activa con `ENGLISH_TUTOR_REQUIRE_UI=1`) la falta de `frontend/dist/index.html` eleva `RuntimeError` accionable (`npm run build`) en lugar de arrancar una app que parece lista y se ve vacía; en desarrollo sigue **fail-open** a propósito (un clon limpio conserva la API). El launcher **no arranca** sin artefacto (`start_backend` eleva `PreparationError`) y `ensure_frontend_dist` distingue «el build falló» de «el build dijo OK y no dejó artefacto». **(B) Descubrimiento de la IP de LAN sin referencias externas (RA-08, P3):** se retira el socket UDP «connect» a `8.8.8.8` de `services/network.py`, `services/tls_cert.py` y `launcher/core.py`; el nuevo `backend/services/net_interfaces.py` enumera las direcciones del propio equipo con un algoritmo **puro** (`select_lan_ipv4`: descarta loopback/link-local/multicast, prefiere privadas, determinista, nunca vacío) y override declarado `ENGLISH_TUTOR_LAN_IP`; el launcher replica el algoritmo (no puede importar el backend) y el contrato se fija por test en los dos lados. **Cascada obligatoria del instrumento RA:** `RUNTIME_TOUCHPOINTS` cambia el punto `lan` de `network.py` (UDP) por `net_interfaces.py`, el par determinista `runtime-audit.{md,json}` se regenera y `RA-RUNTIME-OFFLINE.md` cierra **RA-08** y anota que **RA-06** pierde su excepción (i). **(C) Cobertura CI en Windows (bloque C del dictamen):** de **8 a 11 jobs** — `launcher-windows` (**bloqueante**: `ruff` + 113 casos del launcher en Windows real, stdlib), `product-origin-windows` (**informativo declarado**: build + `ensure_tls_cert` + uvicorn con TLS + PowerShell comprobando HTML/`version`/404, porque instalar `requirements-dev.txt` en Windows depende de ruedas nativas no verificables desde Linux; criterio de promoción a bloqueante escrito en las notas) y `validation-gate` (estático, stdlib). **(D) Arnés de validación:** `scripts/validation_gate.py` con `auto` (10 comprobaciones estáticas → `docs/audit/generated/release-validation.{md,json}`), `record <gate> <pass|fail|skip> --notes` (evidencia en `docs/audit/validation-evidence.json`; **sin notas no hay registro**, y un gate o estado inventado se rechaza) y `status [--strict]` (**la puerta real de V4.0**: sale 1 mientras algún gate no esté en `pass`); los 7 gates (`offline-fisico`, `maquina-limpia`, `launcher-windows`, `dispositivos`, `audio-stt-tts`, `journeys`, `pedagogia`) referencian su protocolo existente en vez de duplicarlo, con runbook en `docs/audit/VALIDATION-RELEASE-V373.md`. **`--strict` no corre en CI** (los gates humanos están `pending` por diseño). **(E) Drift y frontera:** `docs/DEVICE_MATRIX.md` documentaba `https://<ip>:5173` y Vite como runtime **contra lo que V3.72 declaró**; corregido a `:8000` y ampliado con la **matriz de interacción** (touch/tap targets, viewport y scroll, teclado en pantalla, orientación) porque no se puede certificar con capturas; `PREMISAS.md`/`ARQUITECTURA.md`/`README.md` declaran las dos fronteras nuevas y `RC-RUNTIME-PRODUCTO.md` recoge el fail-closed como cierre del P2. **(F) Tests y gates:** backend **2779 passed** (2694 → **+85** en 4 ficheros `*_v373.py`), frontend **699** (83 ficheros, **sin cambios**: V3.73 no toca producto), launcher **113** (93 → **+20**), `ruff`/`tsc` limpios, `npm run build` OK, `check_release_consistency` en los **6 orígenes**, i18n `--strict` con **0 huérfanas** y el CI con **11 jobs**, **11/11 verde** en el [run de publicación 35204203522](https://github.com/jvelasca/english-tutor/actions/runs/35204203522). **Honestidad:** los **7 gates siguen en `pending`** — V3.73 construye el instrumento, **no** hace la validación física (corte de red real RA-05, máquina limpia RB-05, Windows real, móvil real y audio real siguen siendo acción humana); `product-origin-windows` es **informativo**, no bloqueante, y así se declara; el fail-closed cubre el **arranque**, no la ejecución degradada; el descubrimiento local cambia de técnica, no de garantía (`getaddrinfo` puede devolver solo loopback en Windows mal configurados: hay dos vías, override y último recurso `127.0.0.1`); `auto` es **estático** y no sustituye a nada; y **RA-02** (endpoint de Ollama sin declarar en `config.py`), **RA-07** y **RD-05** siguen abiertos. Ver `release-notes-v3.73.0.md`.

- ✅ **V3.72.0 — UX / product completion (2026-09-17)** (**Versión estable `3.72.0`**, app `3.71.0 → 3.72.0`). **Release de PRODUCTO: SIN migración de BD, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco y SIN tocar el currículum.** Cierra los **dos ítems que V3.71 dejó fechados** (`RC-01`: servido del artefacto con Node como requisito de EJECUCIÓN; **RD-04**/mitad de **RA-01**: el vector UI del P1 de TTS/offline), lo que V3.70 asignó a esta fase (**AE-04**, **AE-06**) y la deuda UX declarada (`F2`, `F3`, `F4`, claves i18n huérfanas, «por qué esta actividad»). **(A) Eje UA — servido real de la UI:** el backend monta `frontend/dist` (`StaticFiles` para `/assets` + **fallback SPA** `/{full_path:path}` → `index.html`, **fail-open** si no hay artefacto) y el producto pasa a **un solo proceso HTTPS en `:8000`**: la raíz sirve la UI y los metadatos del servicio se mueven a **`/api`**. El **certificado TLS autofirmado** se genera de forma **determinista e idempotente** (`services/tls_cert.py` + `scripts/ensure_tls_cert.py`, dependencia nueva **`cryptography`**) con los SANs de LAN/mDNS (`localhost`, `127.0.0.1`, `<hostname>`, `<hostname>.local`, IP de LAN), vive en `backend/data/certs/` (**no versionado**) y es obligatorio porque en HTTP por LAN `navigator.mediaDevices` es `undefined` y el micrófono se rompe. El **launcher pasa de dos procesos a uno**: `backend_command()` añade `--ssl-certfile/--ssl-keyfile`, desaparece `start_frontend`, aparece `ensure_frontend_dist()` (`npm run build` **si falta**, con mensaje accionable sin Node) y `allow-firewall.ps1` abre **solo el 8000**; el CI gana el job **`product-origin`** (uvicorn + TLS sobre el `dist` construido, comprobando que la **raíz devuelve el HTML** de la UI). **Node queda como requisito de COMPILACIÓN, no de EJECUCIÓN** (`docs/audit/RC-RUNTIME-PRODUCTO.md` §Cierre). **(B) Eje RD/RA — voz/TTS en la UI:** `voz.ts` devuelve `{voice, degraded}` leyendo `X-TTS-Voice`/`X-TTS-Degraded`, **aborta de verdad** (`AbortController` + timeout) y el consumo se centraliza en `speakWithVoice`, así que todos los puntos de TTS (botones de escucha/speaking, test de micrófono, manos libres, listening, speaking y traductor) informan igual; **aviso no bloqueante** de voz degradada (`useDegradedVoice` + `DegradedVoiceNotice`, con la voz realmente usada) y **consentimiento de descarga** (`useVoiceDownload` + `VoiceDownloadDialog`: ~60 MB, una vez, con Internet, progreso **indeterminado** porque el endpoint es síncrono), con el **auto-download implícito del traductor eliminado**. **(C) Deuda UX:** **F4** (`PanelState`: `loading → error → retry` en `FsrsReviewPanel`, `EvidenceGraphPanel` y `AssessmentLadder`, que antes se tragaban los errores en un `catch` vacío), **F2** (Home lee el readiness **una sola vez**; la tríada queda para Progress), **F3** («estás aquí» con `home.youAreHere` + `home.yourTarget` sobre el nivel estimado), **i18n a 0 huérfanas** (el residuo real eran **10** claves: las 50 de la auditoría incluían falsos positivos de familias dinámicas vivas) con el checker en **`--strict` dentro del CI**, y **«por qué esta actividad»** extendido con una sola pieza compartida (`WhyThisActivity`) al **pie «Next» de cada práctica** (`NextStep`) y a las **filas de la cola de repaso**, **sin recalcular señales en el cliente** (premisa 21: sin `why` ni `because` no se pinta nada). **(D) Instrumentos:** **AE-04** divulga en el instrumento y en su contrato que el placement mide **reconocimiento/meta-lenguaje** (no producción); como **no existe pantalla de nivelación**, la divulgación en UI queda declarada con fase (**→ V4.0.x**) y **fijada por un candado-tripwire**. **AE-06** unifica los **umbrales de banda triplicados** en **un solo módulo** (`services/cefr.py`: `BAND_BOUNDARIES` + `level_for_numeric`) preservando la equivalencia (0 desacuerdos, fijado por test), **retira las sub-bandas `+` de la emisión** (siguen como descriptores de contenido y como posición en la escalera) y conserva el emisor de **`pre-a1`**. **(E) Tests y gates:** backend **2694 passed** (2643 → **+51**), frontend **699** (83 ficheros, 661 → **+38**), launcher **93** (75 → **+18**, en CI), `ruff`/`tsc` limpios, `npm run build` OK, `check_release_consistency` en los **6 orígenes**, `check_beta_v3`/`content_validation`/`transfer_validation` OK, i18n `--strict` con **0 huérfanas** y el CI con **8 jobs**, **8/8 verde** en el [run de publicación 35196758388](https://github.com/jvelasca/english-tutor/actions/runs/35196758388) (el nuevo `product-origin` se estrena ahí). Humo local equivalente verificado: `/` → HTML, assets 200, fallback SPA 200 y `/api/*` inexistente → 404. **Honestidad:** el certificado es **autofirmado** (el usuario acepta el aviso del navegador; sin HTTPS no hay micrófono en LAN), **`RC-01` se cierra como «Node no es requisito de EJECUCIÓN»** —no como «producto empaquetado»: el `dist` no se versiona y hay que compilarlo—, el **progreso de descarga de voz es indeterminado** (no hay progreso real que consumir), **AE-04 no se muestra en UI** porque no hay superficie que enseñe el resultado del placement, y **RA-05** (corte de red real), **RB-05** (máquina limpia) y **G5** (matriz de dispositivos) siguen siendo **acción humana**. Ver `release-notes-v3.72.0.md`.

- ✅ **V3.71.0 — Runtime real, offline verificado e instalación limpia (2026-09-16)** (**Versión estable `3.71.0`**, app `3.70.0 → 3.71.0`). **Release de VERIFICACIÓN con endurecimiento mínimo: SIN migración, SIN bump de `GENERATOR_VERSION` ni `DECISION_POLICY_VERSION`, SIN tocar el banco, SIN tocar el currículum y SIN capacidad pedagógica nueva.** V3.69 validó la arquitectura del motor adaptativo y V3.70 midió su pedagogía; V3.71 baja al **suelo físico** en **seis ejes** (`RE` gates/CI/deriva · `RA` offline real · `RB` instalación limpia · `RC` runtime de producto y salud honesta · `RD` dependencias ocultas y degradación · `RF` síntesis). **(A) Eje RE:** el launcher era el **único subsistema sin gate** (75 tests no corrían en CI y `BETA_GATES.md` daba ✅ igualmente); nuevo job `launcher` (lint + tests, con los mismos pins que el backend) ⇒ **CI 7/7**, y **cuatro derivas documentales** corregidas y anotadas sin reescribir el histórico —árbol del launcher en `ARQUITECTURA.md`; **modelo por defecto** (`PREMISAS.md`/`README.md` declaraban `qwen3.5:9b`, que el código **veta**: es `llama3.1:8b`); ✅ falsos de `BETA_GATES.md`; matriz de dispositivos declarada verde con `DEVICE_MATRIX.md` **10/10 en ⬜**—, fijadas por **8 tests** que fallan si vuelven. **(B) Eje RD — el hallazgo central:** se cierra el **P1 de TTS/offline diferido cuatro veces desde V3.46**; el `timeout` de la descarga de voces era **código muerto** (`urllib.request.urlretrieve` **no acepta** `timeout`), así que la **única dependencia de Internet en ruta de producto** (un `POST /api/tts` de un idioma sin voz instalada) estaba **sin límite** y su degradación era **muda**; ahora hay `urlopen` con timeout **real** (15 s por operación de socket), descarga **atómica** y verificación del tamaño contra `Content-Length`, y la degradación es **observable** (log + cabeceras `X-TTS-Voice`/`X-TTS-Degraded`, expuestas por CORS). De los tres vectores del P1 quedan **2 cerrados** y el de **UI re-declarado con fase a V3.72**; con ello caen **RA-03** y parte de **RA-01**. **(C) Eje RC:** medida la frontera real —la UI la sirve el **dev server de Vite** y el backend **no** sirve `frontend/dist`— se **declara con condición de salida** (decisión A) que **Node + npm son requisito de EJECUCIÓN**, fijado por test; y caen **dos defectos reales**: el sondeo de Ollama **no tenía cota** mientras el launcher solo espera **1,5 s** (un Ollama **lento** se disfrazaba de «backend caído» ⇒ `LLM_PING_TIMEOUT_SECONDS = 1.0`) y el indicador de cabecera leía `/api/health`, que responde **200 siempre**, así que decía «Conectado» con la BD o el modelo caídos (ahora **Conectado / Degradado / Desconectado**, leyendo la dependencia que gatea `/ready`). **(D) Eje RB:** la **voz con la que la app da clase** (`en_US-lessac-medium`) **no estaba en el catálogo curado**, así que `ensure_voice_for_language("en")` **salía sin intentar la descarga** y en una instalación limpia el **español sí se auto-descargaba y el inglés no** (falsado empíricamente); el bootstrap deja de usar `urlretrieve` y URLs propias (delega en el catálogo y hereda el endurecimiento de RD), gana **verificación previa de solo lectura** (`download_models.py --check`/`--json`, que distingue **descarga** de **local** y declara Ollama `exists=None` porque no puede comprobarlo) y el `README.md` gana el **`ollama pull llama3.1:8b`** explícito y la nota de que `backend/models/` no se versiona (~1,1 GB). **(E) Eje RA:** subcomando **de solo lectura** `runtime-audit` (determinista, con guard de que no escribe en `data/` ni `curriculum/`) + protocolo de los **12 flujos**; identifica **3 dependencias de Internet no declaradas en tiempo de uso** y mide las propiedades positivas (el frontend **no** tiene CDN ni websockets externos; **ninguna otra primitiva de red** en producto). **(F) Eje RF (`docs/audit/RF-SINTESIS-RUNTIME-V371.md`):** **P0 = 0 · P1 = 1 (cerrado) · P2 = 15 · P3 = 14**; de los 15 P2, **8 cerrados**, **3 declarados con fase o condición de salida**, **3 abiertos que exigen acción humana** (RA-02 endpoint de Ollama sin declarar en `config.py`, RA-05 corte de red, G5 matriz de dispositivos) y **1 parcial** (RA-01). **Tests:** backend **2643 passed** (2600 → **+43**), frontend **661** (76 ficheros), launcher **75** y **ahora en CI**, `ruff` limpio en backend y launcher, `tsc` limpio. **Honestidad:** **no hay verificación offline en vivo** (los 12 flujos son estáticos; en CI es imposible por la premisa 12, así que cualquier «offline verde» en CI sería simulado), **no se ha probado en una máquina físicamente limpia** (RB-05), **no se prueba hardware móvil real**, **no se mide calidad acústica**, y «offline verificado» debe leerse como «sin dependencias de Internet declaradas», **no** como «producto empaquetado y distribuible» (**el runtime sigue siendo el de desarrollo**, RC-01). Ver `release-notes-v3.71.0.md`.
