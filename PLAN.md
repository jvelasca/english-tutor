...
- **Cierre del motor adaptativo (V3.68, 2026-09-15):** tras V3.68 el **diseño**
  ...
  **diff de código de producto CERO** y **cinco hallazgos aceptados como deuda**
  en la release note) → **V3.70** auditoría pedagógica (CEFR/competencias)
  **(siguiente)** → **V3.71** runtime/offline/instalación → **V3.72**
...
| V3.69 E2E + Adaptive Engine Validation (batería E01–E19; validación, no capacidad nueva) | `agentes/v369-e2e-adaptive-validation.md` | ✔ hecho (2026-09-15; release `v3.69.0`; **20 tests E2E** + 2 specs de navegador; diff de producto **CERO**; 5 hallazgos aceptados) |
| Auditoría EXTERNA de DISEÑO de V3.69 (pre-implementación: briefing + batería E01–E19 + derivación P2/P3) | `agentes/auditoria-externa-v369.md` | ⏳ lanzada (informe esperado en `docs/audit/Z-AUDITORIA-DISENO-V369.md`); **la implementación ya está cerrada**, así que el dictamen se aplicará como corrección documental o se trasladará a V3.70+ |
| Auditoría EXTERNA de V3.69 (post-implementación, sobre el código y los 5 hallazgos) | `docs/audit/Y-AUDITORIA-TOTAL-V368.md` (precedente) | ⏳ **siguiente paso declarado**: auditar `v3.68.0 → v3.69.0` en GitHub |
...
- **⏳ V3.70 — Auditoría pedagógica (CEFR / competencias)**: **siguiente
  incremento** declarado por el roadmap (auditoría `X` de V3.67, confirmada por
  la `Y` de V3.68). Con el motor adaptativo **cerrado y ahora validado** por
  V3.69 (la cadena completa demostrada por HTTP y **diff de producto cero**), el
  foco deja de ser el núcleo adaptativo y pasa a ser el **rigor pedagógico de lo
  que ese núcleo sirve**: política pedagógica formal, error mastery, vocabulario
  (exposure/production/mastery), listening como competencia, CEFR basado en
  evidencia y pronunciación fonémica (tracks P1–P6 de M13). Roadmap posterior
  (**sin cambios**): **V3.71** runtime/offline/instalación → **V3.72**
  UX/product completion → **V3.73** auditoría final técnica → **V4.0** release
  final («English Tutor, primera versión completa y estable») y a partir de ahí
  `V4.0.x` (mantenimiento y calibración, **no** construcción del núcleo).
...
- **✅ V3.69 — E2E + Adaptive Engine Validation (el circuito completo de una
  ...
  **Regla dura declarada (auditoría `Y` §28):** *V3.69 no debe introducir
  ...
  insuficiente ⇒ no se tocó producción** (el diff de producto es **cero**); las
