...
- **🔄 V3.71 — Runtime / offline / instalación**: **incremento en curso**
  ...
  **P1 de TTS/offline** diferido en 4 sitios desde V3.46 — su `timeout` de
  ...
  falta el corte de red real): `docs/audit/RA-RUNTIME-OFFLINE.md`. **Eje RC
  cerrado con una declaración (2026-09-16)**: medido el runtime real —la UI la
  sirve el **dev server de Vite** (`npm run dev`) y el backend **no** sirve
  `frontend/dist`, así que **Node + npm son requisito de EJECUCIÓN**— y, por la
  decisión **(A)**, eso se **declara con condición de salida** (se reevalúa en
  V3.72/V3.73 con el eje RB) en lugar de implementarse. En el camino caen dos
  defectos reales: el sondeo de Ollama **no tenía cota** y el launcher solo
  espera 1,5 s, de modo que un Ollama **lento** (cargando modelo) se disfrazaba
  de «backend caído» → `LLM_PING_TIMEOUT_SECONDS = 1.0`; y el indicador de
  cabecera preguntaba a `/api/health`, que responde **200 siempre** que el
  proceso esté vivo, así que con Ollama o la BD caídos seguía en verde diciendo
  «Conectado» (ahora **tres** estados: Conectado / **Degradado** /
  Desconectado). Evidencia en `docs/audit/RC-RUNTIME-PRODUCTO.md`. Los ejes
  **RB/RF siguen pendientes**. Los
