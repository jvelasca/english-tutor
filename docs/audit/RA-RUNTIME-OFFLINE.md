...
| **RA-03** | **P2** | **La caché negativa de voces es volátil (300 s, en memoria).** Con la red cortada y una voz ausente, cada reintento vuelve a intentar la descarga con `timeout=300.0`: la petición de TTS puede quedarse **colgada hasta 5 minutos** en lugar de fallar rápido, y el olvido se produce en cada reinicio del proceso | `services/tts.py:56-57,176-179`; `services/voice_downloads.py:113` | Cachear el fallo de forma persistente o usar un timeout corto para el intento optimista; devolver error explícito y ofrecer la descarga como acción del usuario | **abierto** (solapa con RD) |
| **RA-04** | **P2** | **En un clon limpio el manifiesto está VACÍO y no hay runbook que lo diga.** `backend/models/` está en `.gitignore`, así que hacen falta ~1,1 GB de descargas (Piper EN+ES 120 MB, Whisper 927 MB) más el `ollama pull` del modelo por defecto. Sin ellos, RA-01 convierte el primer uso en descargas implícitas | `.gitignore:15`; E3 (`Ausentes: 0` **solo** en este equipo) | Cerrarlo en el eje **RB** (instalación limpia reproducible) con el runbook y la verificación previa | **trasladado a RB** |
...
**Hallazgos: P0 = 0 · P1 = 0 · P2 = 5 (1 trasladado a RB, 1 bloquea el cierre) · P3 = 2 (1 positivo, 1 deuda).**
